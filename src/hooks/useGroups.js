...
export function useGroups({ session, flash, dbErr, setDbError, gainXp, hydrateMerge }) {
  ...
  const onGroupPost = (gid, payload) => { gainXp(8); flash(t("toast.publishedInGroup")); postsApi.create({ text: payload.text, images: payload.images || (payload.imageUrl ? [payload.imageUrl] : null), poll: payload.poll, quiz: payload.quiz, group_id: gid, video_url: payload.video || null, bg: payload.bg || null, feeling: payload.feeling || null, location: payload.location || null, tagged: payload.tagged || null }).then(() => mergeGroupPosts(gid)).catch(dbErr("ჯგუფის პოსტი")); };
