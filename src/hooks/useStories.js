...
export function useStories({ flash, dbErr, setDbError, gainXp }) {
  ...
  const onAddStory = (item) => { if (!item.image && !(item.text && item.text.trim()) && !(item.stickers && item.stickers.length)) { flash(t("toast.storyNeedsContent")); return; } setStoryEditorOpen(false); gainXp(8); flash(item.is_broadcast ? t("toast.storyBroadcast") : item.close_friends ? t("toast.storyAddedCloseFriends") : t("toast.storyAdded")); storiesApi.create({ image_url: item.image || null, filter: item.filter, text: item.text, stickers: item.stickers, close_friends: item.close_friends, is_broadcast: item.is_broadcast }).then(loadStories).catch(dbErr("story")); };
