...
import {
  ...
  C, SH, card, DISPLAY, GBRAND, Mono, GRADS, hashIdx, Pic, Title, Chips, Empty, MUSIC_GENRES, ME,
} from "./core";
    ...
    <div className="fixed inset-0 z-[60] flex sm:items-center justify-center items-end" style={{ background: "rgba(6,7,12,.55)", backdropFilter: "blur(4px)", height: vph ? vph + "px" : "100dvh" }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="w-full sm:max-w-[520px] sm:rounded-3xl rounded-t-3xl overflow-y-auto" style={{ background: C.surface, boxShadow: SH.pop, maxHeight: vph ? vph + "px" : "88vh" }}>
        <div className="flex items-center justify-between px-4 py-3.5 sticky top-0" style={{ background: C.surface, borderBottom: `1px solid ${C.lineSoft}` }}>
          ...
          <span className="font-bold" style={{ color: C.ink, fontFamily: DISPLAY }}>{initial ? "სიმღერის რედაქტირება" : "სიმღერის დამატება"}</span>
        ...
        <div className="p-4 space-y-3.5" style={{ paddingBottom: "calc(var(--mz-nav, 64px) + 1.25rem)" }}>
          ...
          <div className="flex gap-2 items-center flex-wrap">
            ...
            {!cover && <span className="text-[12px]" style={{ color: C.faint }}>დაამატე ყდის ფოტო (არასავალდებულო) 🎵</span>}
          </div>
          <div className="flex gap-2 items-center flex-wrap">
            <input ref={audioRef} type="file" accept="audio/*" hidden onChange={pickAudio} />
            <button onClick={() => audioRef.current && audioRef.current.click()} disabled={uploadingAudio} className="rounded-xl flex flex-col items-center justify-center shrink-0 active:scale-95" style={{ width: 72, height: 72, background: C.accentSoft, color: C.accentText }}>{uploadingAudio ? <span className="text-[10px] font-bold">…</span> : <><Upload size={20} /><span className="text-[10px] font-bold mt-0.5 text-center">აუდიო</span></>}</button>
            ...
            {!audio && <span className="text-[12px]" style={{ color: C.faint }}>ატვირთე აუდიო ფაილი (სავალდებულო)</span>}
          </div>
          <input autoFocus value={title} onChange={(e) => setTitle(e.target.value)} placeholder="სიმღერის სახელი" className="w-full px-3.5 py-3 rounded-xl outline-none text-[15px]" style={{ background: C.surfaceMuted, color: C.ink, border: `1px solid ${C.line}` }} />
          <input value={artist} onChange={(e) => setArtist(e.target.value)} placeholder="შემსრულებელი" className="w-full px-3.5 py-3 rounded-xl outline-none text-[15px]" style={{ background: C.surfaceMuted, color: C.ink, border: `1px solid ${C.line}` }} />
...
const SORTS = [["new", "ახალი"], ["popular", "პოპულარული"]];
...
export function MusicPage({ songs, nowPlaying, isPlaying, onPlay, onNew, onEdit, onDelete, onUpload, onUploadAudio }) {
  ...
  return (
    <div className="pb-28 md:pb-10">
      <div className="flex items-center justify-between px-4 pt-5 pb-3"><Title>მუსიკა</Title>
        <button onClick={() => setCreating(true)} className="flex items-center gap-1.5 px-3.5 py-2 rounded-full text-sm font-bold text-white active:scale-95" style={{ backgroundImage: GBRAND, boxShadow: SH.glow }}><Plus size={16} /> სიმღერა</button>
      </div>
      <div className="px-4 pb-3"><input value={artistQ} onChange={(e) => setArtistQ(e.target.value)} placeholder="ძებნა სახელით ან შემსრულებლით…" className="w-full px-4 py-2.5 rounded-full text-[14px] outline-none" style={{ background: C.surfaceMuted, color: C.ink, border: `1px solid ${C.line}` }} /></div>
      <div className="flex gap-1 px-4 pb-3">{SORTS.map(([k, l]) => <button key={k} onClick={() => setSort(k)} className="flex-1 py-2 rounded-xl text-[13px] font-bold transition" style={sort === k ? { background: C.accentSoft, color: C.accentText } : { background: C.surfaceMuted, color: C.muted }}>{l}</button>)}</div>
      ...
      {list.length ? (
            ...
            return (
              <div key={s.id} className="flex items-center gap-3 p-2.5" style={card()}>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[12.5px] truncate" style={{ color: C.muted }}>{s.artist || "უცნობი შემსრულებელი"}</div>
                  <div className="flex items-center gap-1.5 mt-0.5 text-[11px]" style={{ color: C.faint }}><span className="rounded px-1.5 py-0.5" style={{ background: C.accentSoft, color: C.accentText, fontSize: 10, fontWeight: 700 }}>{s.genre}</span><Mono>{s.plays} მოსმენა</Mono></div>
                </div>
        ...
        <Empty icon={Music} t="სიმღერა ვერ მოიძებნა" s="დაამატე პირველი სიმღერა ან შეცვალე ფილტრი." />
      )}
      ...
      {confirmDel && <ConfirmDialog title="სიმღერის წაშლა" msg="ნამდვილად წაშლი ამ სიმღერას?" onCancel={() => setConfirmDel(null)} onConfirm={() => { onDelete(confirmDel.id); setConfirmDel(null); }} />}
    </div>
  );
}
