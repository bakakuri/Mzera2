...
export function CreateSheet({ onClose, onPost, live, onUpload }) {
  const [text, setText] = useState(""); const [pics, setPics] = useState([]); const [poll, setPoll] = useState(null); const [schedAt, setSchedAt] = useState("");
  ...
  const submit = () => onPost(text.trim(), poll ? [] : pics, validPoll ? { options: poll.filter(o => o.trim()).map(t => ({ text: t.trim(), votes: 0 })), voted: null } : null, schedAt ? new Date(schedAt).toISOString() : null);
