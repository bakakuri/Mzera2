...
function LinkPreview({ text }) {
  ...
  const [data, setData] = useState(url && _ogCache.has(url) ? _ogCache.get(url) : null);
  useEffect(() => {
    if (!url || _ogCache.has(url)) return;
    let cancel = false;
    fetch("https://api.microlink.io/?url=" + encodeURIComponent(url)).then(r => r.json()).then(j => { const d = (j && j.status === "success" && j.data) ? { title: j.data.title, desc: j.data.description, img: j.data.image && j.data.image.url, pub: j.data.publisher } : {}; _ogCache.set(url, d); if (!cancel) setData(d); }).catch(() => { _ogCache.set(url, {}); if (!cancel) setData({}); });
    return () => { cancel = true; };
  }, [url]);
  if (!url) return null;
