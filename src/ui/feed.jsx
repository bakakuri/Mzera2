...
export function Lightbox({ images, start, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-center justify-center overflow-hidden" style={{ background: "rgba(0,0,0,.97)" }} onTouchStart={onStart} onTouchMove={onMove} onTouchEnd={onEnd} onClick={(e) => { if (e.target === e.currentTarget && scale <= 1) onClose(); }}>
      <button onClick={onClose} aria-label="დახურვა" className="absolute top-4 right-4 z-10 rounded-full flex items-center justify-center active:scale-90" style={{ width: 40, height: 40, background: "rgba(255,255,255,.15)", color: "#fff" }}><X size={22} /></button>
      ...
      {images.length > 1 && scale <= 1 && <>
        <button onClick={() => go(-1)} disabled={idx === 0} aria-label="წინა" className="hidden sm:flex absolute left-3 top-1/2 rounded-full items-center justify-center" style={{ width: 42, height: 42, background: "rgba(255,255,255,.15)", color: "#fff", opacity: idx === 0 ? 0.3 : 1, transform: "translateY(-50%)" }}><ArrowLeft size={22} /></button>
        <button onClick={() => go(1)} disabled={idx === images.length - 1} aria-label="შემდეგი" className="hidden sm:flex absolute right-3 top-1/2 rounded-full items-center justify-center" style={{ width: 42, height: 42, background: "rgba(255,255,255,.15)", color: "#fff", opacity: idx === images.length - 1 ? 0.3 : 1, transform: "translateY(-50%) rotate(180deg)" }}><ArrowLeft size={22} /></button>
      </>}
    </div>
  );
}
...
function PostImages({ images, pid }) {
  ...
  return (
    <>
      {images.length === 1
        ? <button onClick={() => setLb(0)} aria-label={t("a11y.viewPhoto")} className="block w-full active:opacity-95"><Pic src={images[0]} grad={GRADS[hashIdx(pid, GRADS.length)]} w={800} style={{ aspectRatio: "1 / 1" }} className="w-full" /></button>
        : <div className="relative">
            <div onScroll={onScroll} className="flex overflow-x-auto snap-x snap-mandatory no-scrollbar">{images.map((u, i) => <button key={i} onClick={() => setLb(i)} aria-label={t("a11y.viewPhoto")} className="shrink-0 w-full snap-center active:opacity-95" style={{ aspectRatio: "1 / 1" }}><Pic src={u} grad={GRADS[hashIdx(pid + i, GRADS.length)]} w={800} style={{ aspectRatio: "1 / 1" }} className="w-full h-full" /></button>)}</div>
...
export function LinkPreview({ text, compact }) {
  ...
  const wrap = compact ? "mt-1.5" : "mx-4 mb-3";
  if (data && (data.title || data.img)) return (
    <button onClick={open} className={wrap + " block w-full text-left rounded-2xl overflow-hidden active:opacity-90"} style={{ border: `1px solid ${C.line}`, background: C.surfaceMuted }}>
  ...
  return (
    <button onClick={open} className={wrap + " flex items-center gap-2.5 w-full text-left rounded-xl px-3 py-2.5 active:opacity-80"} style={{ border: `1px solid ${C.line}`, background: C.surfaceMuted }}>
...
// a function (not a precomputed object) so labels stay reactive to a
// language change — see the notifVerb() comment in ui/core.jsx.
function getPromoMeta() {
  return {
    group: { icon: Users, label: t("promo.group") },
    film: { icon: Clapperboard, label: t("promo.film") },
    song: { icon: Music, label: t("promo.song") },
    game: { icon: Gamepad2, label: t("promo.game") },
    market: { icon: ShoppingBag, label: t("promo.market") },
    forum: { icon: MessageSquare, label: t("promo.forum") },
    reel: { icon: Film, label: t("promo.reel") },
    map: { icon: Map, label: t("promo.map") },
    languages: { icon: Languages, label: t("promo.languages") },
  };
}
    ...
    <article className="overflow-hidden" style={{ ...card(), overflow: (menu || shareMenu || reactOpen) ? "visible" : "hidden", opacity: (isMine && (post.publicStatus === "pending" || post.publicStatus === "rejected")) ? 0.55 : 1 }}>
      ...
      <div className="flex items-center gap-3 px-4 pt-4 pb-3">
        ...
        <div className="min-w-0 flex-1 leading-tight">
          ...
          {(post.feeling || (post.tagged && post.tagged.length) || post.location) && <div className="text-[12.5px] mt-0.5 truncate" style={{ color: C.muted }}>{post.feeling && <span>{t("post.feels")} {post.feeling}</span>}{post.tagged && post.tagged.length > 0 && <span>{post.feeling ? " · " : ""}{(() => {
            const name = USERS[post.tagged[0]] ? USERS[post.tagged[0]].name.split(" ")[0] : t("post.someone");
            const extra = post.tagged.length - 1;
            if (LANG === "ka") return `${name}${extra > 0 ? ` და კიდევ ${extra}` : ""}-თან`;
            return `${t("post.taggedWithPre")}${name}${extra > 0 ? ` ${t("post.andMore")} ${extra} ${t("post.more")}` : ""}`;
          })()}</span>}{post.location && <span>{(post.feeling || (post.tagged && post.tagged.length)) ? " · " : ""}📍 {post.location}</span>}</div>}
          <div className="truncate mt-0.5 flex items-center gap-1"><Handle h={u.handle} t={post.time} />{post.edited && <span style={{ color: C.faint, fontSize: 11 }}>· {t("post.edited")}</span>}</div>
        </div>
        <div className="relative">
          <button onClick={() => setMenu(m => !m)} aria-label={t("a11y.moreOptions")} className="rounded-full p-1.5 hover:opacity-60" style={{ color: C.faint }}><MoreHorizontal size={20} /></button>
          {menu && (<><div className="fixed inset-0 z-20" onClick={() => setMenu(false)} /><div className="absolute right-0 top-9 z-30 rounded-2xl py-1.5 w-52" style={{ background: C.surface, border: `1px solid ${C.line}`, boxShadow: SH.pop }}>{isMine ? <><button onClick={() => { setEditing(true); setEditText(post.text || ""); setMenu(false); }} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm hover:opacity-70" style={{ color: C.ink2 }}><Settings size={16} /> {t("post.edit")}</button><button onClick={() => { setMenu(false); if (window.confirm(t("post.deleteConfirm"))) onDelete && onDelete(post.id); }} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm hover:opacity-70" style={{ color: C.like }}><Trash2 size={16} /> {t("action.delete")}</button></> : <>{onFavorite && <button onClick={() => { onFavorite(post.authorId); setMenu(false); }} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm hover:opacity-70" style={{ color: isFavorite ? C.accent : C.ink2 }}><Star size={16} fill={isFavorite ? C.accent : "none"} /> {isFavorite ? t("post.unfavorite") : t("post.favorite")}</button>}{onHide && <button onClick={() => { onHide(post.id); setMenu(false); }} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm hover:opacity-70" style={{ color: C.ink2 }}><Eye size={16} /> {t("post.hide")}</button>}{onSeeLess && <button onClick={() => { onSeeLess(post.authorId); setMenu(false); }} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm hover:opacity-70" style={{ color: C.ink2 }}><Minus size={16} /> {t("post.seeLess")}</button>}<button onClick={() => { onReport("post", post.id); setMenu(false); }} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm hover:opacity-70" style={{ color: C.ink2 }}><Flag size={16} /> {t("post.report")}</button>{isAdmin && <button onClick={() => { onRemove(post.id); setMenu(false); }} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm hover:opacity-70" style={{ color: C.like }}><Trash2 size={16} /> {t("post.adminDelete")}</button>}</>}</div></>)}
        </div>
      </div>
      {editing ? <div className="px-4 pb-3"><textarea value={editText} onChange={e => setEditText(e.target.value)} rows={3} aria-label={t("a11y.edit")} className="w-full px-3 py-2.5 rounded-xl text-[15px] outline-none resize-none" style={{ background: C.surfaceMuted, color: C.ink, border: `1px solid ${C.line}` }} autoFocus /><div className="flex gap-2 mt-2"><button onClick={() => { onEdit && onEdit(post.id, editText.trim()); setEditing(false); }} className="px-4 py-2 rounded-lg text-sm font-bold text-white active:scale-95" style={{ backgroundImage: GBRAND }}>{t("action.save")}</button><button onClick={() => setEditing(false)} className="px-4 py-2 rounded-lg text-sm font-bold" style={{ background: C.surfaceMuted, color: C.muted }}>{t("action.cancel")}</button></div></div>
      ...
      ); })() : post.sharedId ? <div className="mx-4 mb-3 rounded-2xl px-4 py-5 text-center text-[13px]" style={{ border: `1px solid ${C.line}`, background: C.surfaceMuted, color: C.faint }}>{t("post.deletedPlaceholder")}</div> : null}
      ...
      <div className="flex items-center gap-1 px-3 pt-2.5 pb-1.5">
        ...
        <div className="relative">
          {shareMenu && (<><div className="fixed inset-0 z-10" onClick={() => setShareMenu(false)} /><div className="absolute bottom-11 left-0 z-20 rounded-2xl py-1.5 w-56" style={{ background: C.surface, border: `1px solid ${C.line}`, boxShadow: SH.pop }}><button onClick={() => { setShareMenu(false); setRepostText(""); setRepostOpen(true); }} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm font-semibold hover:opacity-70" style={{ color: C.ink2 }}><span style={{ fontSize: 16 }}>🔁</span> {t("post.repostToWall")}</button><button onClick={() => { setShareMenu(false); share(); }} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm font-semibold hover:opacity-70" style={{ color: C.ink2 }}><Send size={16} /> {t("post.shareLink")}</button></div></>)}
          <button onClick={() => setShareMenu(m => !m)} className="flex items-center gap-1.5 px-2 py-1.5 rounded-full active:scale-90 transition" style={{ color: shared ? C.online : C.ink2 }}>{shared ? <Check size={20} /> : <Send size={20} />}<Mono className="text-sm font-semibold">{shared ? t("post.shared") : post.shares}</Mono></button>
        </div>
        <div className="flex-1" />
        <button onClick={() => onSave(post.id)} aria-label={post.savedByMe ? t("a11y.unsave") : t("a11y.save")} aria-pressed={!!post.savedByMe} className="px-2 py-1.5 rounded-full active:scale-90" style={{ color: post.savedByMe ? C.accent : C.ink2 }}><Bookmark size={21} fill={post.savedByMe ? C.accent : "none"} /></button>
      </div>
      ...
      {open && (
        <div className="px-4 pb-1" style={{ borderTop: `1px solid ${C.lineSoft}` }}>
          {(() => {
            ...
            return <>
              {cmts.length > 1 && <div className="flex gap-1.5 pt-2.5">{[["top", t("sort.popular")], ["new", t("sort.new")], ["old", t("sort.old")]].map(([k, l]) => <button key={k} onClick={() => setCsort(k)} className="px-2.5 py-1 rounded-full text-[12px] font-bold" style={csort === k ? { background: C.accentSoft, color: C.accentText } : { color: C.faint }}>{l}</button>)}</div>}
              <div className="pt-1.5">{tops.map(c => <div key={c.id}>{row(c, false)}{repliesOf(c.id).map(r => row(r, true))}</div>)}</div>
            </>;
          })()}
          <div className="py-2">
            {replyTo && <div className="flex items-center gap-1.5 mb-1.5 pl-1"><span className="text-[12px]" style={{ color: C.accent }}>{t("comment.replyToPre")}{replyName}{t("comment.replyToPost")}</span><button onClick={() => { setReplyTo(null); setReplyName(""); }} style={{ color: C.faint, fontSize: 13 }}>✕</button></div>}
            <div className="flex items-center gap-2"><Avatar id={ME} size={30} /><input ref={cInRef} value={draft} onChange={e => setDraft(e.target.value)} onKeyDown={e => e.key === "Enter" && send()} placeholder={replyTo ? t("comment.replyPh") : t("comment.addPh")} className="flex-1 bg-transparent text-[14px] outline-none" style={{ color: C.ink }} />{draft.trim() && <button onClick={send} className="text-sm font-bold" style={{ color: C.accent }}>{t("action.publish")}</button>}</div>
          </div>
        </div>
      )}
      {reactorsOpen && createPortal((
        <div className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center" style={{ background: "rgba(0,0,0,.5)" }} onClick={() => setReactorsOpen(false)}>
          <div className="w-full sm:max-w-sm sm:rounded-3xl rounded-t-3xl max-h-[70vh] overflow-y-auto pb-4" style={{ background: C.surface }} onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-3.5 sticky top-0 z-10" style={{ background: C.surface, borderBottom: `1px solid ${C.lineSoft}` }}><span className="font-bold text-[15px]" style={{ color: C.ink }}>{t("reactions.title")}</span><button onClick={() => setReactorsOpen(false)} aria-label={t("a11y.close")} style={{ color: C.muted }}><X size={20} /></button></div>
            {reactors == null ? <div className="flex justify-center py-10"><div style={{ width: 24, height: 24, border: `3px solid ${C.lineSoft}`, borderTopColor: C.accent, borderRadius: "50%", animation: "spin 0.8s linear infinite" }} /></div> : reactors.length === 0 ? <Empty icon={Heart} t={t("comment.noneYet")} s="" /> : <div className="px-2 py-1">{reactors.map(r => { if (r.user) mergeProfile(r.user); const id = r.user_id; return (<button key={id} onClick={() => { setReactorsOpen(false); onOpenProfile(id); }} className="w-full flex items-center gap-3 px-3 py-2.5 active:opacity-60"><Avatar id={id} size={38} /><div className="flex-1 text-left min-w-0"><div className="font-bold text-[14px] truncate" style={{ color: C.ink }}>{USERS[id] ? USERS[id].name : "—"}</div><Mono style={{ fontSize: 11.5, color: C.faint }}>@{USERS[id] ? USERS[id].handle : ""}</Mono></div><span style={{ fontSize: 22 }}>{r.emoji || "❤️"}</span></button>); })}</div>}
          </div>
        </div>
      ), document.body)}
      {repostOpen && createPortal((() => {
        ...
        <div className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center" style={{ background: "rgba(0,0,0,.55)" }} onClick={() => setRepostOpen(false)}>
          <div className="w-full sm:max-w-md sm:rounded-3xl rounded-t-3xl max-h-[85vh] overflow-y-auto" style={{ background: C.surface }} onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-4 py-3.5 sticky top-0 z-10" style={{ background: C.surface, borderBottom: `1px solid ${C.lineSoft}` }}><button onClick={() => setRepostOpen(false)} aria-label={t("a11y.close")} style={{ color: C.muted }}><X size={22} /></button><span className="font-bold" style={{ color: C.ink, fontFamily: DISPLAY }}>{t("post.shareTitle")}</span><button onClick={() => { if (onRepost) onRepost(target.id, repostText.trim()); setRepostOpen(false); }} className="px-4 py-1.5 rounded-full text-sm font-bold text-white active:scale-95" style={{ backgroundImage: GBRAND }}>{t("post.shareAction")}</button></div>
            <div className="p-4">
              <textarea value={repostText} onChange={e => setRepostText(e.target.value)} rows={2} placeholder={t("comment.addOptionalPh")} className="w-full resize-none px-3.5 py-3 rounded-xl outline-none text-[15px] mb-3" style={{ background: C.surfaceMuted, color: C.ink, border: `1px solid ${C.line}` }} autoFocus />
...
export function StoryViewer({ story, onClose, onDone, flash, onReport }) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center" style={{ background: "rgba(6,7,12,.96)" }}>
      <div ref={modalRef} tabIndex={-1} role="dialog" aria-modal="true" className="relative w-full max-w-[440px] h-full sm:h-[92vh] sm:rounded-3xl overflow-hidden" style={{ outline: "none" }}>
        ...
        <button className="absolute left-0 top-0 w-1/3 h-full" onClick={() => setIdx(i => Math.max(0, i - 1))} aria-label={t("a11y.previous")} />
        <button className="absolute right-0 top-0 w-1/3 h-full" onClick={() => idx < items.length - 1 ? setIdx(i => i + 1) : (onDone(story.id), onClose())} aria-label={t("a11y.next")} />
        ...
        <div className="absolute bottom-0 inset-x-0 p-3 flex items-center gap-2">
          <input value={reply} onChange={e => setReply(e.target.value)} onFocus={() => setPaused(true)} onBlur={() => { if (!reply.trim()) setPaused(false); }} onKeyDown={e => { if (e.key === "Enter") send(); }} placeholder={t("comment.replyShortPh")} className="flex-1 px-4 py-2.5 rounded-full text-sm text-white bg-transparent outline-none" style={{ border: "1px solid rgba(255,255,255,.5)" }} />
          {reply.trim() ? <button onClick={send} aria-label={t("a11y.send")} className="rounded-full flex items-center justify-center active:scale-90 shrink-0" style={{ width: 42, height: 42, backgroundImage: GBRAND, color: "#fff", boxShadow: SH.glow }}><Send size={18} /></button> : <button onClick={toggleLike} aria-label={liked ? t("a11y.unlike") : t("a11y.like")} className="active:scale-90 shrink-0" style={{ color: liked ? "#ff3b5c" : "#fff" }}><Heart size={28} fill={liked ? "#ff3b5c" : "none"} /></button>}
        </div>
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 z-[60] flex sm:items-center justify-center items-end" style={{ background: "rgba(6,7,12,.55)", backdropFilter: "blur(4px)" }} onClick={onClose}>
      <div ref={modalRef} tabIndex={-1} role="dialog" aria-modal="true" onClick={e => e.stopPropagation()} className="w-full sm:max-w-[520px] sm:rounded-3xl rounded-t-3xl max-h-[90vh] overflow-y-auto" style={{ background: C.surface, boxShadow: SH.pop, outline: "none" }}>
        ...
        <div className="px-4 pb-5 pt-1">
          ...
          {panel === "tag" && <div className="mt-2.5 max-h-44 overflow-y-auto rounded-xl" style={{ background: C.surfaceMuted }}>{(taggable || []).length === 0 ? <div className="text-[13px] text-center py-4" style={{ color: C.faint }}>{t("compose.noTaggable")}</div> : (taggable || []).map(id => { const tu = USERS[id]; if (!tu) return null; const on = tagged.includes(id); return <button key={id} onClick={() => setTagged(t => on ? t.filter(x => x !== id) : [...t, id])} className="w-full flex items-center gap-2.5 px-3 py-2 active:opacity-70"><Avatar id={id} size={32} /><span className="flex-1 text-left text-[14px] font-semibold truncate" style={{ color: C.ink }}>{tu.name}</span>{on && <Check size={18} style={{ color: C.accent }} />}</button>; })}</div>}
          {panel === "loc" && <input value={loc} onChange={e => setLoc(e.target.value)} placeholder={t("compose.wherePh")} className="w-full mt-2.5 px-3.5 py-2.5 rounded-xl outline-none text-[14px]" style={{ background: C.surfaceMuted, color: C.ink, border: `1px solid ${C.line}` }} />}
          <button onClick={() => setWantPublic(v => !v)} className="w-full flex items-center justify-between px-3.5 py-3 rounded-xl mt-2.5 active:scale-[.99]" style={{ background: wantPublic ? C.accentSoft : C.surfaceMuted, border: `1px solid ${wantPublic ? C.accent : C.line}` }}>
            <div className="text-left"><div className="text-[14px] font-bold flex items-center gap-1.5" style={{ color: wantPublic ? C.accentText : C.ink2 }}>{t("compose.publicToAll")}</div><div className="text-[11.5px] mt-0.5" style={{ color: C.faint }}>{t("compose.needsApproval")}</div></div>
          ...
          <div className="mt-2.5">
            <div className="flex items-center gap-2 text-[13px] font-semibold mb-1.5" style={{ color: C.muted }}><span style={{ fontSize: 15 }}>📅</span> {t("compose.schedule")} <span className="font-normal" style={{ color: C.faint }}>{t("compose.optional")}</span></div>
            <input type="datetime-local" value={schedAt} onChange={e => setSchedAt(e.target.value)} aria-label={t("compose.schedule")} className="w-full px-3 py-2.5 rounded-xl outline-none text-[14px]" style={{ background: C.surfaceMuted, color: C.ink, border: `1px solid ${C.line}` }} />
            {schedAt && <div className="flex items-center justify-between mt-1.5"><span className="text-[12px]" style={{ color: C.accent }}>{t("compose.willPublishLater")}</span><button onClick={() => setSchedAt("")} className="text-[12px] font-bold" style={{ color: C.faint }}>{t("action.clear")}</button></div>}
          </div>
        </div>
      </div>
    </div>
...
export function StoryEditor({ onClose, onShare, live, onUpload, isAdmin }) {
  const [pic, setPic] = useState(null); const [filter, setFilter] = useState("none"); const [text, setText] = useState(""); const [stickers, setStickers] = useState([]); const [mode, setMode] = useState("text"); const [cf, setCf] = useState(false); const [broadcast, setBroadcast] = useState(false);
  ...
  return (
    <div className="fixed inset-0 z-[62] flex items-center justify-center" style={{ background: "rgba(0,0,0,.96)" }}>
      <div ref={modalRef} tabIndex={-1} role="dialog" aria-modal="true" className="relative w-full max-w-[440px] h-full sm:h-[92vh] sm:rounded-3xl overflow-hidden" style={{ background: "#000", outline: "none" }}>
        ...
        <div className="absolute top-0 inset-x-0 p-3 flex items-center justify-between" style={{ background: "linear-gradient(180deg, rgba(0,0,0,.4), transparent)" }}>
          <button onClick={onClose} aria-label={t("a11y.close")} className="rounded-full flex items-center justify-center active:scale-90" style={{ width: 40, height: 40, background: "rgba(0,0,0,.4)", color: "#fff", backdropFilter: "blur(6px)" }}><X size={22} /></button>
          <div className="flex gap-1.5">{[["filter", t("mode.filter")], ["text", t("mode.text")], ["sticker", t("mode.sticker")]].map(([k, l]) => <button key={k} onClick={() => setMode(k)} className="px-3 py-1.5 rounded-full text-xs font-bold transition" style={mode === k ? { background: "#fff", color: "#000" } : { background: "rgba(255,255,255,.2)", color: "#fff", backdropFilter: "blur(6px)" }}>{l}</button>)}</div>
        </div>
        <div className="absolute bottom-0 inset-x-0 p-3" style={{ background: "linear-gradient(0deg, rgba(0,0,0,.65), transparent)" }}>
          {mode === "filter" && <div className="flex gap-2 overflow-x-auto no-scrollbar mb-3">{getFilters().map(([l, f]) => <button key={l} onClick={() => setFilter(f)} className="shrink-0 flex flex-col items-center gap-1"><div className="rounded-xl overflow-hidden" style={{ width: 54, height: 54, outline: filter === f ? "2.5px solid #fff" : "none", outlineOffset: 2 }}><div style={{ filter: f, width: "100%", height: "100%" }}><Pic src={srcAt(96, 96)} grad={g} className="w-full h-full" /></div></div><span className="text-[10px] text-white">{l}</span></button>)}</div>}
          ...
          {!broadcast && <button onClick={() => setCf(v => !v)} className="w-full mb-2 rounded-xl flex items-center justify-center gap-2 py-2.5 font-bold text-[13px] active:scale-95 transition" style={cf ? { background: "#1f8f4e", color: "#fff" } : { background: "rgba(255,255,255,.18)", color: "#fff", backdropFilter: "blur(6px)" }}>{cf ? <><Star size={15} /> {t("story.closeFriendsOnly")}</> : <><Users size={15} /> {t("story.everyone")}</>}</button>}
          {isAdmin && <button onClick={() => setBroadcast(v => !v)} className="w-full mb-2 rounded-xl flex items-center justify-center gap-2 py-2.5 font-bold text-[13px] active:scale-95 transition" style={broadcast ? { background: "#c0392b", color: "#fff" } : { background: "rgba(255,255,255,.18)", color: "#fff", backdropFilter: "blur(6px)" }}><Globe size={15} /> {broadcast ? t("story.broadcastOn") : t("story.broadcastOff")}</button>}
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => onShare({ image: srcAt(480, 854), filter, text: text.trim(), stickers, close_friends: broadcast ? false : cf, is_broadcast: broadcast })} className="rounded-full px-5 py-3 font-bold text-white flex items-center gap-2 shrink-0 active:scale-95" style={{ backgroundImage: broadcast ? "linear-gradient(135deg,#c0392b,#e74c3c)" : cf ? "linear-gradient(135deg,#1f8f4e,#26b061)" : GBRAND, boxShadow: SH.glow }}>{t("post.shareTitle")} <Send size={17} /></button>
          </div>
        </div>
      </div>
    </div>
