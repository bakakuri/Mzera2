...
export function PostCard({ post, onLike, onReact, onSave, onComment, onPollVote, onTag, onReport, onRemove, onOpenProfile, isAdmin, onEdit, onDelete, onEditComment, onDeleteComment, onLikeComment, onRepost, onReactors }) {
  ...
  const bgPost = post.bg && POST_BGS[post.bg] && post.text && !post.image && !(post.images && post.images.length) && !post.poll && !post.shared;
  ...
  return (
    <article className="overflow-hidden" style={card()}>
      ...
      {post.shared && (() => { const su = USERS[post.shared.authorId]; return (
        <div className="mx-4 mb-3 rounded-2xl overflow-hidden" style={{ border: `1px solid ${C.line}`, background: C.surfaceMuted }}>
          <button onClick={() => su && onOpenProfile(su.id)} className="flex items-center gap-2 px-3 pt-3 pb-2 w-full active:opacity-70"><Avatar id={post.shared.authorId} size={28} /><div className="text-left min-w-0"><div className="font-bold text-[13.5px] truncate" style={{ color: C.ink }}>{su ? su.name : "—"}</div><Mono style={{ fontSize: 11, color: C.faint }}>@{su ? su.handle : ""} · {post.shared.time}</Mono></div></button>
          {post.shared.text && <div className="px-3 pb-2.5 text-[14px] whitespace-pre-wrap" style={{ color: C.ink2, lineHeight: 1.5 }}>{post.shared.text}</div>}
          {post.shared.image && <Pic src={post.shared.image} grad={GRADS[hashIdx(post.shared.id, GRADS.length)]} className="w-full" style={{ maxHeight: 360, objectFit: "cover" }} />}
        </div>
      ); })()}
...
export function CreateSheet({ onClose, onPost, live, taggable, onUpload }) {
  ...
  const pickFile = async (e) => {
    ...
    if (urls.length) { setPics(p => [...p, ...urls].slice(0, 6)); setBg(null); }
    setUploading(false); e.target.value = "";
  };
  const validPoll = poll && poll.filter(o => o.trim()).length >= 2;
  const useBg = bg && !pics.length && !poll;
  const can = text.trim() || pics.length || validPoll;
  const submit = () => onPost(text.trim(), poll ? [] : pics, validPoll ? { options: poll.filter(o => o.trim()).map(t => ({ text: t.trim(), votes: 0 })), voted: null } : null, schedAt ? new Date(schedAt).toISOString() : null, wantPublic, { bg: useBg ? bg : null, feeling, location: loc.trim() || null, tagged });
