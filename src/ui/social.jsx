    ...
    <div className="fixed inset-0 z-[55] md:hidden" style={{ pointerEvents: open ? "auto" : "none" }}>
      ...
      <aside className="absolute top-0 left-0 h-full flex flex-col" style={{ width: "84%", maxWidth: 320, background: C.surface, boxShadow: SH.pop, transform: open ? "translateX(0)" : "translateX(-104%)", transition: "transform .32s cubic-bezier(.4,0,.2,1)" }}>
        <div className="p-5 pb-4" style={{ backgroundImage: GBRAND, color: "#fff" }}>
          <div className="flex items-center justify-between mb-4"><span style={{ fontFamily: DISPLAY, fontWeight: 700, fontSize: 22, letterSpacing: "-0.04em" }}>mzera.</span><button onClick={onClose} aria-label={t("a11y.close")} className="active:scale-90 rounded-full p-1" style={{ background: "rgba(255,255,255,.2)" }}><X size={18} /></button></div>
...
export function NavStrip({ nav, onNav, onCreate, flash, tab, onSettings, onSignOut }) {
  ...
  // rendered 3x back-to-back so there's always a full lap of tiles in either
  // scroll direction; the scroll handler silently jumps by one lap's width
  // whenever the middle lap is left, so it reads as an endless loop.
  const loopedTiles = Array.from({ length: NAVSTRIP_REPEATS }, (_, rep) => tiles.map(tl => ({ ...tl, _key: rep + ":" + tl.key }))).flat();
  // fades the tile row into the page background at both edges, instead of a
  // hard cut, so the strip visibly hints that there's more to scroll to.
