    ...
    <div className="pb-28 md:pb-10">
      ...
      <div className="px-4">
        <div className="flex items-end justify-between" style={{ marginTop: 8 }}>
          <div className="rounded-full" style={{ padding: 6, background: C.paper, boxShadow: "0 8px 20px -6px rgba(0,0,0,.32)" }}>{isMe && onUploadAvatar ? <label style={{ position: "relative", cursor: "pointer", display: "block" }}><input type="file" accept="image/*" style={{ display: "none" }} onChange={e => { const f = e.target.files && e.target.files[0]; if (f) onUploadAvatar(f); e.target.value = ""; }} /><Avatar id={u.id} size={84} fit="contain" /><div style={{ position: "absolute", right: 0, bottom: 0, width: 28, height: 28, borderRadius: "50%", backgroundImage: GBRAND, display: "flex", alignItems: "center", justifyContent: "center", border: `3px solid ${C.paper}` }}><Camera size={14} color="#fff" /></div></label> : <button onClick={() => u.avatar && setAvatarView(true)} className="block active:scale-95" style={{ borderRadius: "50%" }}><Avatar id={u.id} size={84} fit="contain" /></button>}</div>
          {isMe
