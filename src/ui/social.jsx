      ...
      <div className="flex-1 overflow-y-auto px-6">
        {step === 0 ? (
          <div className="max-w-[440px] mx-auto pt-5 pb-4">
            <h1 className="text-[26px] font-bold text-center" style={{ color: C.ink, fontFamily: DISPLAY }}>კეთილი იყოს მობრძანება! 👋</h1>
            <p className="text-center text-[14px] mt-2 mb-7" style={{ color: C.muted }}>დავიწყოთ შენი პროფილის მოწყობით</p>
            <div className="flex flex-col items-center">
              <button onClick={() => fileRef.current && fileRef.current.click()} className="relative active:scale-95"><Avatar id={ME} size={104} /><div style={{ position: "absolute", right: 2, bottom: 2, width: 34, height: 34, borderRadius: "50%", backgroundImage: GBRAND, display: "flex", alignItems: "center", justifyContent: "center", border: `3px solid ${C.paper}` }}><Camera size={17} color="#fff" /></div></button>
              <input ref={fileRef} type="file" accept="image/*" hidden onChange={e => { const f = e.target.files && e.target.files[0]; if (f) onUploadAvatar(f); e.target.value = ""; }} />
              <span className="text-[12.5px] mt-2" style={{ color: C.faint }}>დაამატე პროფილის ფოტო</span>
            </div>
            <div className="mt-6 space-y-3.5">
              <div><label className="text-[12.5px] font-semibold" style={{ color: C.ink2 }}>სახელი</label><input value={name} onChange={e => setName(e.target.value)} placeholder="შენი სახელი" className="w-full mt-1.5 px-4 py-3 rounded-xl text-[15px] outline-none" style={{ background: C.surfaceMuted, color: C.ink }} /></div>
              <div><label className="text-[12.5px] font-semibold" style={{ color: C.ink2 }}>ბიო</label><textarea value={bio} onChange={e => setBio(e.target.value)} rows={2} placeholder="მოგვიყევი შენს შესახებ…" className="w-full mt-1.5 px-4 py-3 rounded-xl text-[15px] outline-none resize-none" style={{ background: C.surfaceMuted, color: C.ink }} /></div>
            </div>
          </div>
        ) : (
          <div className="max-w-[440px] mx-auto pt-5 pb-4">
            <h1 className="text-[24px] font-bold text-center" style={{ color: C.ink, fontFamily: DISPLAY }}>გაჰყევი ხალხს 🌱</h1>
            <p className="text-center text-[14px] mt-2 mb-5" style={{ color: C.muted }}>ფიდი გაცოცხლდება — დააფოლოვე ვინც გაინტერესებს</p>
            {suggested.length ? <div className="space-y-2.5">{suggested.slice(0, 12).map(u => <div key={u.id} className="flex items-center gap-3 p-2.5 rounded-2xl" style={{ background: C.surface, border: `1px solid ${C.line}` }}><Avatar id={u.id} size={46} /><div className="min-w-0 flex-1"><div className="text-[14.5px] font-bold truncate" style={{ color: C.ink }}>{USERS[u.id] ? USERS[u.id].name : ""}</div><Mono className="text-[11.5px] truncate block" style={{ color: C.faint }}>@{USERS[u.id] ? USERS[u.id].handle : ""}</Mono></div><button onClick={() => onToggleFollow(u.id)} className="px-4 py-1.5 rounded-lg text-[12.5px] font-bold active:scale-95" style={following.includes(u.id) ? { background: C.surfaceMuted, color: C.ink } : { backgroundImage: GBRAND, color: "#fff" }}>{following.includes(u.id) ? "✓ მიყვები" : "გაყევი"}</button></div>)}</div> : <div className="text-center py-10 text-[14px]" style={{ color: C.faint }}>ჯერ სხვა მომხმარებლები არ არიან — მოგვიანებით დაბრუნდი 🙂</div>}
          </div>
        )}
      </div>
      <div className="px-6 pt-3" style={{ paddingBottom: "max(1.5rem, env(safe-area-inset-bottom))" }}>
        <button onClick={next} className="w-full py-3.5 rounded-2xl text-[16px] font-bold text-white active:scale-[.98]" style={{ backgroundImage: GBRAND }}>{step === 0 ? "გაგრძელება" : (followedCount > 0 ? "დასრულება 🎉" : "გამოტოვება")}</button>
        {step === 1 && <button onClick={onFinish} className="w-full py-2.5 mt-1 text-[13.5px] font-semibold" style={{ color: C.faint }}>მოგვიანებით</button>}
      </div>
    </div>
  );
}
...
export function SuggestedPeople({ people, isFollowing, onToggle, onDismiss, onOpenProfile }) {
  ...
  return (
    <div style={{ borderBottom: `1px solid ${C.lineSoft}`, background: C.paper }}>
      <div className="flex items-center justify-between px-4 pt-3 pb-1.5"><span className="text-[14px] font-bold" style={{ color: C.ink }}>შეიძლება იცნობდე 👋</span></div>
      <div className="flex gap-2.5 px-4 overflow-x-auto no-scrollbar pb-3">
        {people.map(u => <Tilt key={u.id} max={9} radius={18} touchAction="pan-x" className="shrink-0 p-3 flex flex-col items-center" style={{ width: 144, ...card() }}>
          ...
          <button onClick={() => onToggle(u.id)} className="w-full mt-2.5 py-1.5 rounded-lg text-[12.5px] font-bold transition active:scale-95" style={isFollowing(u.id) ? { background: C.surfaceMuted, color: C.ink } : { backgroundImage: GBRAND, color: "#fff" }}>{isFollowing(u.id) ? "✓ მიყვები" : "გაყევი"}</button>
        </Tilt>)}
      </div>
    </div>
  );
    ...
    <div className="pb-28 md:pb-10">
      ...
      <div className="px-4">
        <div className="flex items-end justify-between" style={{ marginTop: 8 }}>
          <Tilt max={16} radius={999} style={{ padding: 6, background: C.paper, boxShadow: "0 12px 26px -6px rgba(0,0,0,.42)" }}>{isMe && onUploadAvatar ? <label style={{ position: "relative", cursor: "pointer", display: "block" }}><input type="file" accept="image/*" style={{ display: "none" }} onChange={e => { const f = e.target.files && e.target.files[0]; if (f) onUploadAvatar(f); e.target.value = ""; }} /><Avatar id={u.id} size={84} /><div style={{ position: "absolute", right: 0, bottom: 0, width: 28, height: 28, borderRadius: "50%", backgroundImage: GBRAND, display: "flex", alignItems: "center", justifyContent: "center", border: `3px solid ${C.paper}` }}><Camera size={14} color="#fff" /></div></label> : <button onClick={() => u.avatar && setAvatarView(true)} className="block active:scale-95" style={{ borderRadius: "50%" }}><Avatar id={u.id} size={84} /></button>}</Tilt>
          {isMe
...
export function Notifications({ notifs, onOpenProfile, onOpenPost, isFollowing, onToggleFollow }) {
  const verb = { like: t("notif.likedShort"), comment: t("notif.commentedShort"), follow: t("notif.followedShort"), mention: t("notif.tagged"), announcement: t("notif.announcement"), public_approved: t("notif.publicApproved"), public_rejected: t("notif.publicRejected") };
