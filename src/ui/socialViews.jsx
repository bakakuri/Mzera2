      ...
      {seg === "reports" && <div className="px-4">
        {open.length === 0 ? <Empty icon={Check} t="სუფთაა ✨" s="ღია რეპორტი არ არის." /> : (
          <div className="space-y-3">{open.map(r => {
            const target = r.type === "post" ? posts.find(p => p.id === r.targetId) : r.type === "story" ? (stories || []).find(s => s.id === r.targetId) : USERS[r.targetId];
            const badge = r.type === "post" ? { l: "POST", bg: C.accentSoft, c: C.accentText } : r.type === "story" ? { l: "STORY", bg: C.online + "22", c: C.online } : { l: "USER", bg: C.likeSoft, c: C.like };
            return (
