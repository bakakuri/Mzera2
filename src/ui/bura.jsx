...
export function BuraGame({ onExit }) {
  ...
  // bot plays on its turn (lead / defend)
  useEffect(() => {
    if (!g || g.turn !== 1 || (g.phase !== "lead" && g.phase !== "defend")) { setThinking(false); return; }
    setThinking(true);
    const t = setTimeout(() => {
      setG(cur => {
        ...
        if (next === cur) { // never freeze — force a legal move
  ...
  // revealed trick → commit after a beat so the covering cards stay visible
  useEffect(() => {
    if (!g || g.phase !== "reveal") return;
    const t = setTimeout(() => setG(cur => (cur && cur.phase === "reveal" ? resolve(cur) : cur)), 3500);
    return () => clearTimeout(t);
  }, [g]);
  ...
  const toggle = (c) => {
    ...
    setSel(s => {
      ...
      const cap = iDefend ? g.attackCards.length : 5;
      if (s.length >= cap) return s;
      return [...s, id];
    });
  };

  const revealLabel = g.phase === "reveal" ? (g.last && g.last.type === "cover" ? (g.pileWinner === 0 ? "შენ დახურე ✓" : "ბოტმა დახურა") : (g.pileWinner === 0 ? "შენ წაიღე ⬇" : "ბოტმა წაიღო ⬇")) : "";
  const statusText = g.phase === "over" ? "" : g.phase === "reveal" ? revealLabel : iLead ? "შენი სვლა — ჩააგდე 1–5 კარტი (ერთი მასტის)" : iDefend ? "დაიცავი: აირჩიე კარტ(ებ)ი → „სვლა“, ან „გატანება“" : thinking ? "ბოტი ფიქრობს…" : "ბოტის სვლა…";
  ...
  return (
    <div className="fixed inset-0 z-[95] flex flex-col" style={{ background: "radial-gradient(circle at 50% 30%, #14351f, #0c1f13 70%)" }}>
      ...
      <div className="flex items-center gap-2.5 px-4 py-2">
        <div style={{ width: 36, height: 36, borderRadius: "50%", backgroundImage: GBRAND, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 19, border: "2px solid rgba(255,255,255,.2)" }}>🤖</div>
        ...
        <div className="flex">{g.hands[1].map((c, i) => <div key={i} style={{ marginLeft: i ? -14 : 0 }}><CardFace down size="sm" /></div>)}</div>
      </div>
      ...
      <div className="px-4 pt-2" style={{ paddingBottom: "max(1rem, env(safe-area-inset-bottom))" }}>
        ...
        {iDefend && <div className="flex gap-2">
          <button onClick={() => { const ng = take(g); setG(ng); setSel([]); }} className="flex-1 py-3.5 rounded-2xl text-[15px] font-bold active:scale-[.98]" style={{ background: C.surfaceMuted, color: C.ink }}>გატანება</button>
          <button onClick={() => { const ng = cover(g, selCards); setG(ng); setSel([]); }} disabled={!canClose} className="flex-1 py-3.5 rounded-2xl text-[15px] font-bold text-white active:scale-[.98]" style={{ backgroundImage: GBRAND, opacity: canClose ? 1 : 0.4 }}>სვლა</button>
        </div>}
        {g.phase === "reveal" && <button onClick={() => setG(cur => (cur && cur.phase === "reveal" ? resolve(cur) : cur))} className="w-full py-3.5 rounded-2xl text-[15px] font-bold text-white active:scale-[.98]" style={{ backgroundImage: GBRAND }}>გაგრძელება →</button>}
        {!iLead && !iDefend && g.phase !== "over" && g.phase !== "reveal" && <div className="text-center py-3.5 text-[14px] font-semibold" style={{ color: "rgba(255,255,255,.55)" }}>{statusText}</div>}
      </div>
