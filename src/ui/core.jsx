...
export const SH = {
  get card() { return DARK ? "0 1px 2px rgba(0,0,0,.5), 0 6px 16px -4px rgba(0,0,0,.55), 0 22px 48px -22px rgba(0,0,0,.85)" : "0 1px 1.5px rgba(17,19,26,.05), 0 4px 10px -2px rgba(17,19,26,.08), 0 16px 34px -14px rgba(17,19,26,.16)"; },
  get pop() { return DARK ? "0 24px 60px -14px rgba(0,0,0,.9)" : "0 16px 44px -12px rgba(17,19,26,.28)"; },
  get soft() { return DARK ? "0 4px 14px -6px rgba(0,0,0,.6)" : "0 4px 14px -6px rgba(17,19,26,.12)"; },
  glow: "0 2px 0 rgba(64,44,150,.32), 0 8px 22px -6px rgba(103,80,242,.55)",
};

export const card = () => ({
  backgroundImage: DARK ? "linear-gradient(180deg, #181C27, #12151D)" : "linear-gradient(180deg, #FFFFFF, #F7F9FC)",
  border: `1px solid ${C.line}`,
  boxShadow: (DARK ? "inset 0 1px 0 rgba(255,255,255,.05), " : "inset 0 1px 0 rgba(255,255,255,.8), ") + SH.card,
  borderRadius: 18,
});
...
export function Tilt({ children, max = 8, radius = 22, className, style, onClick, touchAction = "pan-y" }) {
  ...
  // touch has no hover concept, and repeatedly rewriting transform mid-touch fights the browser's own scroll/pan gesture — mouse/pen only
  const move = (e) => { if (e.pointerType === "touch") return; const el = ref.current; if (!el) return; const r = el.getBoundingClientRect(); const px = (e.clientX - r.left) / r.width - 0.5; const py = (e.clientY - r.top) / r.height - 0.5; setT(`perspective(1000px) rotateX(${(-py * max).toFixed(2)}deg) rotateY(${(px * max).toFixed(2)}deg) scale(1.012)`); };
  const reset = () => setT("");
...
export async function compressImage(file, maxDim = 1080, quality = 0.7) {
  try {
...
export function mapDbReport(row) {
  if (row.reporter) mergeProfile(row.reporter);
  return { id: row.id, type: row.type, targetId: row.target_id, reason: row.reason || "", reporterId: row.reporter_id, time: timeAgo(row.created_at), status: row.status };
}
...
// ⚡ Supabase image transform — serve small variants for fast feeds.
// Needs Supabase Pro plan; Pic/Avatar fall back to the original URL on error, so nothing breaks on free tier.
export const IMG_TX = true;
export const tx = (url, w, q = 62) => {
  if (!IMG_TX || !url || typeof url !== "string" || !url.includes("/storage/v1/object/public/")) return url;
  const base = url.replace("/storage/v1/object/public/", "/storage/v1/render/image/public/");
  return base + (base.includes("?") ? "&" : "?") + "width=" + w + "&quality=" + q + "&resize=cover";
};
...
export function mapDbReel(row, likedSet, savedSet) {
  ...
  return { id: row.id, authorId: row.author_id, image: row.thumb_url || null, video: row.video_url || null, caption: row.caption || "", audio: row.audio || "original audio", likes, comments, shares: 0, views: row.views || 0, likedByMe: likedSet ? likedSet.has(row.id) : false, savedByMe: savedSet ? savedSet.has(row.id) : false, createdAt: row.created_at };
}
...
export function mapDbSong(row) {
  if (row.author) mergeProfile(row.author);
  return { id: row.id, authorId: row.author_id, title: row.title, artist: row.artist || "", genre: row.genre || "სხვა", cover: row.cover_url || img("song" + row.id, 480, 480), audio: row.audio_url, plays: row.plays || 0, time: timeAgo(row.created_at), createdAt: row.created_at };
}

// compact persistent player — shown in the header wherever there's room,
// regardless of which tab is active, so playback survives navigation
export function MiniPlayer({ song, playing, onToggle, onStop }) {
  if (!song) return null;
  return (
...
export function notifVerb(type) {
  const map = { like: "notif.liked", comment: "notif.commented", reply: "notif.replied", follow: "notif.followed", mention: "notif.tagged", thread_reply: "notif.threadReplied", thread_activity: "notif.threadActivity", profile_view: "notif.profileViewed", reel_like: "notif.reelLiked", reel_comment: "notif.reelCommented", story_like: "notif.storyLiked", story_comment: "notif.storyCommented", post_tag: "notif.postTagged", group_post: "notif.groupPost", group_approved: "notif.groupApproved", event_rsvp: "notif.eventRsvp", birthday: "notif.birthday", level_up: "notif.levelUp", group_join_request: "notif.groupJoinRequest", comment_like: "notif.commentLiked", market_review: "notif.marketReview", repost: "notif.repost", poll_vote: "notif.pollVote", announcement: "notif.announcement", public_approved: "notif.publicApproved", public_rejected: "notif.publicRejected" };
...
export const SettingsSection = ({ title, children }) => <div className="mt-5"><div className="px-4 pb-1.5 text-[12px] font-bold uppercase" style={{ color: C.faint, fontFamily: MONO, letterSpacing: "0.04em" }}>{title}</div><div className="mx-3 rounded-2xl overflow-hidden" style={{ ...card(), borderRadius: 16 }}>{children}</div></div>;
...
export { authApi, profilesApi, postsApi, reactionsApi, commentsApi, followsApi, chatApi, notifsApi, storageApi, storiesApi, reelsApi, marketApi, filmsApi, musicApi, groupsApi, eventsApi, forumApi, highlightsApi, presenceApi, locationsApi, pollsApi, questsApi, xpApi, adminApi, reportsApi, pushApi, languagesApi };
