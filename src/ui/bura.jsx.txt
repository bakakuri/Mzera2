...
export function BuraGame({ onExit }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setG(cur => (cur && cur.phase === "reveal" ? resolve(cur) : cur)), 1100);
    return () => clearTimeout(t);
  }, [g]);
  ...
  const statusText = g.phase === "over" ? "" : g.phase === "reveal" ? revealLabel : iLead ? "შენი სვლა — ჩააგდე კარტ(ებ)ი" : iDefend ? "დაიცავი — სვლა ან გატანება" : thinking ? "ბოტი ფიქრობს…" : "ბოტის სვლა…";
