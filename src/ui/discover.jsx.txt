  ...
  return (
    <div className="relative" style={{ height: "100dvh", background: "#000" }}>
      <div ref={scrollRef} onScroll={onScroll} className="no-scrollbar overflow-y-scroll h-full" style={{ scrollSnapType: "y mandatory" }}>
        {reels.map((r, i) => <ReelCard key={r.id} r={r} onLike={onLike} onSave={onSave} onView={onView} onOpenProfile={onOpenProfile} flash={flash} onComments={onComments} muted={muted} onToggleMute={() => setMuted(m => !m)} priority={Math.abs(i - activeIdx) <= 1} />)}
