...
export function Avatar({ id, size = 40, ring = false, story = false, seen = false, closeFriends = false, fit = "cover" }) {
  ...
  const inner = u.avatar
    ? <img src={fb ? u.avatar : tx(u.avatar, Math.min(280, Math.round(size * 2)))} onError={() => { if (!fb) setFb(true); }} alt="" style={{ width: size, height: size, objectFit: fit, background: fit === "contain" ? `linear-gradient(140deg, ${a}, ${b})` : undefined }} className="rounded-full select-none shrink-0" draggable={false} />
