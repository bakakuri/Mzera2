...
export const np = (o) => ({ id: "p" + pid++, likedByMe: false, savedByMe: false, shares: Math.floor(Math.random() * 40), reported: false, hidden: false, comments: [], ...o });
...
export function Avatar({ id, size = 40, ring = false, story = false, seen = false }) {
...
export function renderText(text, onTag) {
...
export function NewThread({ onClose, onCreate }) {
...
export function NewListing({ onClose, onCreate, live, onUpload }) {
...
export const NOTIF_VERB = { like: "მოიწონა შენი პოსტი ❤️", comment: "დააკომენტარა შენი პოსტი 💬", follow: "გამოგყვა 👤", mention: "მოგიხსენია პოსტში" };
...
export function ReelCard({ r, onLike, onSave, onOpenProfile, flash, onComments, muted, onToggleMute }) {
...
export function ReelCreate({ onClose, onPublish, onUpload, flash }) {
...
export function GroupPost({ p, onOpenProfile }) {
...
export { authApi, profilesApi, postsApi, reactionsApi, commentsApi, followsApi, chatApi, notifsApi, storageApi, storiesApi, reelsApi, marketApi, groupsApi, eventsApi, forumApi, highlightsApi, presenceApi, locationsApi, pollsApi };
export { hasSupabase };
