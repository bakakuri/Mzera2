...
export const CallLayer = forwardRef(function CallLayer({ me, enabled }, ref) {
  ...
  const sendTo = (peerId, kind, data) => {
    let ch = outboundRef.current[peerId];
    if (!ch) { ch = callsApi.channel(peerId); ch.subscribe(); outboundRef.current[peerId] = ch; }
    const msg = { type: "broadcast", event: "signal", payload: { kind, from: me, to: peerId, ...data } };
    try { ch.send(msg); } catch (e) { setTimeout(() => { try { ch.send(msg); } catch (e2) {} }, 350); }
  };
  ...
  const cleanup = () => {
    ...
    Object.values(outboundRef.current).forEach(ch => { try { ch.unsubscribe(); } catch (e) {} });
  ...
  const makePc = (peerId) => {
    ...
    pc.ontrack = (e) => {
      ...
      if (!stream) return;
      const isVideo = callRef.current && callRef.current.video;
      if (isVideo) { if (remoteVideoRef.current) remoteVideoRef.current.srcObject = stream; }
  ...
  const onSignal = async (p) => {
    ...
    if (p.kind === "offer") {
      ...
      if (c && pcRef.current && c.peer.id === p.from && !pcRef.current.currentRemoteDescription) { try { await pcRef.current.setRemoteDescription(p.sdp); } catch (e) {} }
    } else if (p.kind === "ice") {
