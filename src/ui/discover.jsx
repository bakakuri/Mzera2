  ...
  if (th) {
    ...
    return (
      <div className="pb-36 md:pb-10">
        <div className="flex items-center gap-3 px-4 py-3 sticky top-0 z-10" style={{ background: C.paper + "e6", backdropFilter: "blur(12px)" }}><button onClick={() => setOpenId(null)} aria-label={t("a11y.back")} style={{ color: C.ink2 }}><ArrowLeft size={22} /></button><span className="font-bold flex-1" style={{ color: C.ink, fontFamily: DISPLAY }}>{t("word.thread")}</span>{isAdmin && <><button onClick={() => onSetThreadPinned(th.id, !th.pinned)} className="px-2.5 py-1.5 rounded-full text-[12px] font-bold" style={th.pinned ? { backgroundImage: GBRAND, color: "#fff" } : { background: C.surfaceMuted, color: C.ink2 }}>{th.pinned ? t("forum.unpin") : t("forum.pin")}</button><button onClick={() => onSetThreadLocked(th.id, !th.locked)} className="px-2.5 py-1.5 rounded-full text-[12px] font-bold" style={{ background: C.surfaceMuted, color: th.locked ? C.like : C.ink2 }}>{th.locked ? t("forum.unlock") : t("forum.lock")}</button></>}{th.authorId === ME && <><button onClick={() => setEditing(th)} aria-label={t("a11y.edit")} className="active:scale-90" style={{ color: C.ink2 }}><Pencil size={19} /></button><button onClick={() => setConfirmDel(true)} aria-label={t("action.delete")} className="active:scale-90" style={{ color: C.like }}><Trash2 size={19} /></button></>}</div>
        <div className="px-3">
...
export function Market({ listings, onSave, onNew, onMessage, onOpenProfile, flash, live, onOrder, getReviews, onAddReview, onUpload, onEdit, onDelete, sentinelRef, hasMore, loadingMore, pendingOpen, clearPending }) {
  ...
  if (it) {
    ...
    return (
      <div className="pb-36 md:pb-10">
        <div className="flex items-center gap-3 px-4 py-3 sticky top-0 z-10" style={{ background: C.paper + "e6", backdropFilter: "blur(12px)" }}><button onClick={() => setOpenId(null)} aria-label={t("a11y.back")} style={{ color: C.ink2 }}><ArrowLeft size={22} /></button><span className="font-bold truncate" style={{ color: C.ink, fontFamily: DISPLAY }}>{it.title}</span><div className="flex-1" />{it.sellerId === ME && <><button onClick={() => setEditing(it)} aria-label={t("a11y.edit")} className="active:scale-90" style={{ color: C.ink2 }}><Pencil size={20} /></button><button onClick={() => setConfirmDel(true)} aria-label={t("action.delete")} className="active:scale-90" style={{ color: C.like }}><Trash2 size={20} /></button></>}<button onClick={() => onSave(it.id)} aria-label={it.savedByMe ? t("a11y.unsave") : t("a11y.save")} style={{ color: it.savedByMe ? C.accent : C.ink2 }}><Bookmark size={22} fill={it.savedByMe ? C.accent : "none"} /></button></div>
        ...
        <div className="fixed bottom-0 inset-x-0 z-30 md:static flex gap-2 px-4 py-3 max-w-[600px] mx-auto" style={{ background: C.surface, borderTop: `1px solid ${C.line}`, paddingBottom: "calc(var(--mz-nav, 64px) + 0.6rem)" }}>
          <button onClick={() => onMessage(u.id)} aria-label={t("a11y.send")} className="px-4 py-3 rounded-2xl font-bold flex items-center gap-2" style={{ background: C.surfaceMuted, color: C.ink2 }}><Send size={19} /></button>
  ...
  return (
    <div className="relative" style={{ height: "100dvh", background: C.mapBase }}>
      ...
      <div className="absolute top-0 inset-x-0 p-3 flex items-center gap-2" style={{ zIndex: 1000, background: `linear-gradient(180deg, ${C.paper}cc, transparent)` }}>
        <button onClick={onMenu} aria-label={t("a11y.menu")} className="md:hidden rounded-full flex items-center justify-center active:scale-90" style={{ width: 42, height: 42, background: C.surface, boxShadow: SH.card, color: C.ink2 }}><Menu size={22} /></button>
      ...
      <div className="absolute right-3 flex flex-col gap-2" style={{ zIndex: 1000, bottom: sf ? 270 : 160 }}>
        <div className="rounded-2xl overflow-hidden" style={{ background: C.surface, boxShadow: SH.card }}>
          <button onClick={() => mapObj.current && mapObj.current.zoomIn()} aria-label={t("a11y.zoomIn")} className="flex items-center justify-center active:scale-90" style={{ width: 44, height: 44, color: C.ink2, borderBottom: `1px solid ${C.lineSoft}` }}><Plus size={20} /></button>
          <button onClick={() => mapObj.current && mapObj.current.zoomOut()} aria-label={t("a11y.zoomOut")} className="flex items-center justify-center active:scale-90" style={{ width: 44, height: 44, color: C.ink2 }}><Minus size={20} /></button>
        </div>
        <button onClick={recenter} aria-label={t("a11y.recenter")} className="rounded-2xl flex items-center justify-center active:scale-90" style={{ width: 44, height: 44, background: C.surface, boxShadow: SH.card, color: C.accent }}><Navigation size={19} /></button>
      </div>
      ...
      {sf && (
        <div className="absolute left-3 right-3 p-3.5 flex items-center gap-3" style={{ zIndex: 1000, bottom: 90, ...card() }}>
          ...
          {sf.user_id !== ME && <button onClick={() => onMessage(sf.user_id)} aria-label={t("a11y.send")} className="rounded-full flex items-center justify-center active:scale-90" style={{ width: 44, height: 44, backgroundImage: GBRAND, color: "#fff", boxShadow: SH.glow }}><Send size={18} /></button>}
          <button onClick={() => setSel(null)} aria-label={t("a11y.close")} className="rounded-full flex items-center justify-center" style={{ width: 32, height: 32, color: C.faint }}><X size={18} /></button>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="relative" style={{ height: "100dvh", background: "#000" }}>
      ...
      <div className="absolute top-0 inset-x-0 p-3 flex items-center justify-between pointer-events-none">
        <button onClick={onMenu} aria-label={t("a11y.menu")} className="md:hidden rounded-full flex items-center justify-center active:scale-90 pointer-events-auto" style={{ width: 42, height: 42, background: "rgba(255,255,255,.15)", backdropFilter: "blur(8px)", color: "#fff" }}><Menu size={22} /></button>
        <div className="flex items-center gap-4">{tabBtn("foryou", t("reels.forYou"))}{tabBtn("recent", t("reels.recent"))}</div>
        <button onClick={onCreate} aria-label={t("a11y.createReel")} className="rounded-full flex items-center justify-center active:scale-90 pointer-events-auto" style={{ width: 42, height: 42, background: "rgba(255,255,255,.15)", backdropFilter: "blur(8px)", color: "#fff" }}><Plus size={24} /></button>
      </div>
      {soundView && (
        <div className="absolute inset-0 z-20 flex flex-col" style={{ background: "rgba(0,0,0,.93)", backdropFilter: "blur(6px)" }}>
          <div className="flex items-center gap-3 px-4 pb-3" style={{ paddingTop: 18 }}>
            <button onClick={() => setSoundView(null)} aria-label={t("a11y.back")} className="rounded-full flex items-center justify-center active:scale-90" style={{ width: 38, height: 38, background: "rgba(255,255,255,.15)", color: "#fff" }}><ArrowLeft size={20} /></button>
...
export function Groups({ groups, events, onJoin, onRsvp, onOpenProfile, onMessage, live, onGroupPost, onUpload, onUploadVideo, onCreateGroup, onCreateEvent, pendingOpen, clearPending, pendingEvent, clearPendingEvent, onEditPost, onDeletePost, onEditGroup, onDeleteGroup, onEditEvent, onDeleteEvent, allPosts, loadGroupPosts, loadMembers, onApproveMember, onKickMember, onBanMember, onUnbanMember, onTransferOwnership, onSetGroupPrivate, postProps, taggable }) {
  ...
  const [groupQ, setGroupQ] = useState(""); const [groupCatFilter, setGroupCatFilter] = useState("ყველა");
  useEffect(() => { if (gOpen && loadGroupPosts) loadGroupPosts(gOpen); setMgmtOpen(false); setGroupView("posts"); }, [gOpen]);
  ...
  const [cCat, setCCat] = useState(GROUP_CATS[1]);
  ...
  const resetCreate = () => { setCreating(false); setEditId(null); setCName(""); setCAbout(""); setCLoc(""); setCDate(""); setCCover(null); setCCat(GROUP_CATS[1]); };
  // not useModalA11y: that hook's effect only runs once on mount, but Groups
  // (the whole tab, not this sheet) is what's actually mounted/unmounted —
  // scoping the effect to [creating] instead makes it fire exactly when the
  // sheet opens/closes, same as every other modal in this codebase gets by
  // being its own separately-mounted component.
  const createModalRef = useRef(null);
  useEffect(() => {
    if (!creating) return;
    const prevFocused = document.activeElement;
    if (createModalRef.current) createModalRef.current.focus();
    const onKey = (ev) => { if (ev.key === "Escape") resetCreate(); };
  ...
  const submitCreate = () => {
    ...
    } else if (seg === "groups") onCreateGroup({ name: cName.trim(), about: cAbout.trim(), category: cCat, coverUrl: cCover });
  ...
  const gq = groupQ.trim().toLowerCase();
  const filteredGroups = groups.filter(gr => (groupCatFilter === "ყველა" || gr.cat === groupCatFilter) && (!gq || gr.name.toLowerCase().includes(gq) || (gr.about || "").toLowerCase().includes(gq)));
  ...
  if (g) {
    ...
    return (
      <div className="pb-28 md:pb-10">
        <div className="flex items-center gap-3 px-4 py-3 sticky top-0 z-10" style={{ background: C.paper + "e6", backdropFilter: "blur(12px)" }}><button onClick={() => setGOpen(null)} aria-label={t("a11y.back")} style={{ color: C.ink2 }}><ArrowLeft size={22} /></button><span className="font-bold truncate flex-1" style={{ color: C.ink, fontFamily: DISPLAY }}>{g.name}</span>{g.createdBy === ME && <><button onClick={() => openEdit("group", g)} aria-label={t("a11y.edit")} className="active:scale-90" style={{ color: C.ink2 }}><Pencil size={19} /></button><button onClick={() => setDelTarget({ type: "group", id: g.id })} aria-label={t("action.delete")} className="active:scale-90" style={{ color: C.like }}><Trash2 size={19} /></button></>}</div>
        ...
        {g.owner && mgmtOpen && <div className="mx-3 mt-3 rounded-2xl overflow-hidden" style={{ border: `1px solid ${C.line}`, background: C.surface }}>
          ...
          <div className="px-4 py-2.5"><div className="text-[12px] font-bold mb-2" style={{ color: C.muted }}>{t("group.membersCountPre")}{mem.filter(m => m.status === "approved").length})</div>{mem.filter(m => m.status === "approved").map(m => <div key={m.user_id} className="flex items-center gap-2.5 py-1.5"><Avatar id={m.user_id} size={34} /><div className="flex-1 min-w-0"><span className="text-[14px] font-semibold truncate block" style={{ color: C.ink }}>{USERS[m.user_id] ? USERS[m.user_id].name : "—"}</span>{m.role === "owner" && <span className="text-[11px] font-bold" style={{ color: C.accent }}>{t("group.creatorBadge")}</span>}</div>{m.role !== "owner" && <div className="flex items-center gap-1.5 shrink-0"><button onClick={() => setTransferTarget(m.user_id)} className="px-2.5 py-1.5 rounded-full text-[11.5px] font-bold" style={{ background: C.accentSoft, color: C.accentText }}>{t("group.makeOwner")}</button><button onClick={() => onKickMember(g.id, m.user_id)} className="px-2.5 py-1.5 rounded-full text-[12px] font-bold" style={{ background: C.surfaceMuted, color: C.ink2 }}>{t("group.kick")}</button><button onClick={() => onBanMember(g.id, m.user_id)} className="px-2.5 py-1.5 rounded-full text-[12px] font-bold" style={{ background: C.surfaceMuted, color: C.like }}>{t("group.ban")}</button></div>}</div>)}</div>
          {mem.filter(m => m.status === "banned").length > 0 && <div className="px-4 py-2.5" style={{ borderTop: `1px solid ${C.lineSoft}` }}><div className="text-[12px] font-bold mb-2" style={{ color: C.muted }}>{t("group.bannedHeader")}{mem.filter(m => m.status === "banned").length})</div>{mem.filter(m => m.status === "banned").map(m => <div key={m.user_id} className="flex items-center gap-2.5 py-1.5"><Avatar id={m.user_id} size={34} /><span className="flex-1 text-[14px] font-semibold truncate" style={{ color: C.ink }}>{USERS[m.user_id] ? USERS[m.user_id].name : "—"}</span><button onClick={() => onUnbanMember(g.id, m.user_id)} className="px-3 py-1.5 rounded-full text-[12.5px] font-bold" style={{ background: C.surfaceMuted, color: C.ink2 }}>{t("group.unban")}</button></div>)}</div>}
        </div>}
        {!locked && <div className="flex gap-1 mx-3 mt-5 mb-2 p-1 rounded-2xl" style={{ background: C.surfaceMuted }}>{[["posts", t("group.postsHeader")], ["photos", t("group.photosTab")]].map(([k, l]) => <button key={k} onClick={() => setGroupView(k)} className="flex-1 py-2 rounded-xl text-sm font-bold transition" style={groupView === k ? { background: C.surface, color: C.accent, boxShadow: SH.card } : { color: C.muted }}>{l}</button>)}</div>}
        ...
        {transferTarget && <ConfirmDialog title={t("group.transferConfirmTitle")} msg={t("group.transferConfirmMsg")} confirmText={t("group.makeOwner")} onCancel={() => setTransferTarget(null)} onConfirm={() => { onTransferOwnership(g.id, transferTarget); setTransferTarget(null); setMgmtOpen(false); }} />}
      </div>
    );
  }
  if (e) {
    ...
    return (
      <div className="pb-28 md:pb-10">
        <div className="flex items-center gap-3 px-4 py-3 sticky top-0 z-10" style={{ background: C.paper + "e6", backdropFilter: "blur(12px)" }}><button onClick={() => setEOpen(null)} aria-label={t("a11y.back")} style={{ color: C.ink2 }}><ArrowLeft size={22} /></button><span className="font-bold truncate flex-1" style={{ color: C.ink, fontFamily: DISPLAY }}>{t("word.event")}</span>{e.hostId === ME && <><button onClick={() => openEdit("event", e)} aria-label={t("a11y.edit")} className="active:scale-90" style={{ color: C.ink2 }}><Pencil size={19} /></button><button onClick={() => setDelTarget({ type: "event", id: e.id })} aria-label={t("action.delete")} className="active:scale-90" style={{ color: C.like }}><Trash2 size={19} /></button></>}</div>
  ...
  return (
    <div className="pb-28 md:pb-10">
      ...
      {seg === "groups" ? (
        ...
        <div className="px-4 mb-3"><div className="flex items-center gap-2 px-3 py-2 rounded-full" style={{ background: C.surfaceMuted }}><Search size={16} style={{ color: C.faint }} /><input value={groupQ} onChange={e => setGroupQ(e.target.value)} placeholder={t("group.searchPh")} className="flex-1 bg-transparent text-[14px] outline-none" style={{ color: C.ink }} /></div></div>
        <Chips items={GROUP_CATS} value={groupCatFilter} onChange={setGroupCatFilter} />
        {filteredGroups.length === 0 ? <Empty icon={Users} t={t("picker.notFound")} s="" /> :
        <div className="space-y-3 px-3">{filteredGroups.map(gr => (
      ...
      {creating && (
        <div className="fixed inset-0 z-[60] flex items-end md:items-center justify-center" style={{ background: "rgba(0,0,0,.45)" }} onClick={resetCreate}>
          <div ref={createModalRef} tabIndex={-1} role="dialog" aria-modal="true" className="w-full max-w-[480px] rounded-t-3xl md:rounded-3xl p-5" style={{ background: C.surface, maxHeight: "88vh", overflowY: "auto", paddingBottom: "calc(env(safe-area-inset-bottom) + 5.5rem)", outline: "none" }} onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4"><h2 className="text-[18px]" style={{ color: C.ink, fontFamily: DISPLAY, fontWeight: 700 }}>{editId ? (seg === "groups" ? t("group.editTitle") : t("event.editTitle")) : (seg === "groups" ? t("group.newTitle") : t("event.newTitle"))}</h2><button onClick={resetCreate} aria-label={t("a11y.close")} style={{ color: C.faint }}><X size={22} /></button></div>
            ...
            {seg === "groups" && !editId && <div className="flex gap-1.5 flex-wrap mb-3">{GROUP_CATS.slice(1).map(c => <button key={c} onClick={() => setCCat(c)} className="px-3 py-1.5 rounded-full text-sm font-semibold transition" style={cCat === c ? { background: C.accentSoft, color: C.accentText } : { background: C.surfaceMuted, color: C.muted }}>{c}</button>)}</div>}
