  ...
  return (
    <div className="relative" style={{ height: "100dvh", background: "#000" }}>
      ...
      <div className="absolute top-0 inset-x-0 p-3 flex items-center justify-between pointer-events-none">
        ...
        <div className="flex items-center gap-4">{tabBtn("foryou", "შენთვის")}{tabBtn("recent", "ბოლო")}</div>
      ...
      {soundView && (
        <div className="absolute inset-0 z-20 flex flex-col" style={{ background: "rgba(0,0,0,.93)", backdropFilter: "blur(6px)" }}>
          <div className="flex items-center gap-3 px-4 pb-3" style={{ paddingTop: 18 }}>
            ...
            <div className="min-w-0"><div className="text-white font-bold text-[16px] truncate" style={{ fontFamily: DISPLAY }}>🎵 {soundView}</div><div className="text-white/60 text-[12px]">{soundReels.length} reel ამ ხმით</div></div>
          </div>
          <div className="flex-1 overflow-y-auto p-1 grid grid-cols-3 gap-1 content-start">
            {soundReels.map(r => <button key={r.id} onClick={() => setSoundView(null)} className="relative overflow-hidden active:opacity-80" style={{ aspectRatio: "9/16", borderRadius: 8 }}><Pic src={r.image} grad={GRADS[hashIdx(r.id, GRADS.length)]} style={{ aspectRatio: "9/16" }} /><span className="absolute bottom-1 left-1 flex items-center gap-0.5 text-white text-[11px] font-bold" style={{ filter: "drop-shadow(0 1px 2px rgba(0,0,0,.7))" }}><Play size={11} fill="#fff" /> {kfmt(r.views || 0)}</span></button>)}
          </div>
          <button onClick={() => { setSoundView(null); onCreate(); }} className="m-4 py-3 rounded-2xl text-[15px] font-bold text-white flex items-center justify-center gap-2 active:scale-[.98]" style={{ backgroundImage: GBRAND }}><Film size={18} /> ამ ხმით გადაღება</button>
        </div>
      )}
    </div>
  );
    ...
    return (
      <div className="pb-28 md:pb-10">
        ...
        <div className="px-4 pt-4">
          ...
          {!g.owner && <button onClick={() => onJoin(g.id)} className="w-full mt-3.5 py-2.5 rounded-xl text-sm font-bold transition active:scale-[.98]" style={(g.joined || g.pending) ? { background: C.surface, color: C.ink, border: `1px solid ${C.line}` } : { backgroundImage: GBRAND, color: "#fff", boxShadow: SH.glow }}>{g.joined ? "✓ წევრი ხარ" : g.pending ? "⏳ მოთხოვნა გაგზავნილია" : g.isPrivate ? "🔒 შესვლის მოთხოვნა" : "შეუერთდი ჯგუფს"}</button>}
          {g.owner && <button onClick={() => setMgmtOpen(o => !o)} className="w-full mt-3.5 py-2.5 rounded-xl text-sm font-bold transition active:scale-[.98] flex items-center justify-center gap-2" style={{ background: mgmtOpen ? C.accentSoft : C.surface, color: mgmtOpen ? C.accentText : C.ink, border: `1px solid ${mgmtOpen ? C.accent : C.line}` }}><Settings size={16} /> ჯგუფის მართვა{g.pendingCount > 0 ? ` · ${g.pendingCount} ⏳` : ""}</button>}
        </div>
        {g.owner && mgmtOpen && <div className="mx-3 mt-3 rounded-2xl overflow-hidden" style={{ border: `1px solid ${C.line}`, background: C.surface }}>
          <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: `1px solid ${C.lineSoft}` }}><div><div className="text-[14px] font-bold" style={{ color: C.ink }}>🔒 დახურული ჯგუფი</div><div className="text-[12px]" style={{ color: C.faint }}>ახალი წევრები დადასტურებას საჭიროებენ</div></div><Switch on={!!g.isPrivate} onClick={() => onSetGroupPrivate(g.id, !g.isPrivate)} /></div>
          {mem.filter(m => m.status === "pending").length > 0 && <div className="px-4 py-2.5" style={{ borderBottom: `1px solid ${C.lineSoft}` }}><div className="text-[12px] font-bold mb-2" style={{ color: C.muted }}>მოთხოვნები ({mem.filter(m => m.status === "pending").length})</div>{mem.filter(m => m.status === "pending").map(m => <div key={m.user_id} className="flex items-center gap-2.5 py-1.5"><Avatar id={m.user_id} size={34} /><span className="flex-1 text-[14px] font-semibold truncate" style={{ color: C.ink }}>{USERS[m.user_id] ? USERS[m.user_id].name : "—"}</span><button onClick={() => onApproveMember(g.id, m.user_id)} className="px-3 py-1.5 rounded-full text-[12.5px] font-bold text-white" style={{ backgroundImage: GBRAND }}>დადასტურება</button><button onClick={() => onKickMember(g.id, m.user_id)} className="px-2.5 py-1.5 rounded-full text-[12.5px] font-bold" style={{ background: C.surfaceMuted, color: C.like }}>უარი</button></div>)}</div>}
          <div className="px-4 py-2.5"><div className="text-[12px] font-bold mb-2" style={{ color: C.muted }}>წევრები ({mem.filter(m => m.status !== "pending").length})</div>{mem.filter(m => m.status !== "pending").map(m => <div key={m.user_id} className="flex items-center gap-2.5 py-1.5"><Avatar id={m.user_id} size={34} /><div className="flex-1 min-w-0"><span className="text-[14px] font-semibold truncate block" style={{ color: C.ink }}>{USERS[m.user_id] ? USERS[m.user_id].name : "—"}</span>{m.role === "owner" && <span className="text-[11px] font-bold" style={{ color: C.accent }}>შემქმნელი 👑</span>}</div>{m.role !== "owner" && <button onClick={() => onKickMember(g.id, m.user_id)} className="px-3 py-1.5 rounded-full text-[12px] font-bold" style={{ background: C.surfaceMuted, color: C.like }}>გაგდება</button>}</div>)}</div>
        </div>}
        ...
        {locked ? <div className="mx-3 rounded-2xl px-5 py-10 text-center" style={{ background: C.surface, border: `1px solid ${C.line}` }}><div style={{ fontSize: 34 }}>🔒</div><div className="text-[15px] font-bold mt-2" style={{ color: C.ink }}>დახურული ჯგუფი</div><div className="text-[13px] mt-1" style={{ color: C.faint }}>პოსტების სანახავად გახდი წევრი</div></div>
        ...
        <div className="space-y-4 px-3">{gPosts.length ? gPosts.map(p => <PostCard key={p.id} post={p} {...postProps} />) : <Empty icon={MessageSquare} t="ჯერ პოსტი არ არის" s={canPost ? "დაწერე პირველი 👆" : ""} />}</div></>}
      </div>
    );
  }
  if (e) {
