...
export function cover(state, cards) {
  ...
  const s = clone(state);
  const won = s.attackCards.concat(cards);
  s.hands[def] = without(s.hands[def], cards);
  s.captured[def] += handPts(won);
  s.capturedCards[def] = s.capturedCards[def].concat(won);
  s.attackCards = [];
  s.last = { type: "cover", by: def, cards: cards.slice() };
  s.attacker = def;            // winner leads next
  finishTrick(s, def);
  return s;
}
...
export function take(state) {
  ...
  s.captured[atk] += handPts(s.attackCards);
  s.capturedCards[atk] = s.capturedCards[atk].concat(s.attackCards);
  s.last = { type: "take", by: 1 - atk, cards: s.attackCards.slice() };
  s.attackCards = [];
  // attacker keeps the lead
  finishTrick(s, atk);
  return s;
}
