...
export const posts = {
  ...
  mySaves: async () => {
    const sb = need();
    const uid = (await sb.auth.getUser()).data.user.id;
    const { data, error } = await sb.from("post_saves").select("created_at, post:posts!post_saves_post_id_fkey(*, author:profiles!posts_author_id_fkey(*))").eq("user_id", uid).order("created_at", { ascending: false });
    if (error) throw error;
  ...
  create: async ({ text, imageUrl, images, poll }) => {
    const sb = need();
    const uid = (await sb.auth.getUser()).data.user.id;
    const arr = (images && images.length) ? images : (imageUrl ? [imageUrl] : null);
    const { data, error } = await sb
      .from("posts")
      .insert({ author_id: uid, text, image_url: arr ? arr[0] : null, images: arr, has_poll: !!poll })
      .select()
      .single();
    if (error) throw error;
    if (poll && data) {
...
export const comments = {
  ...
  forPosts: async (ids) => {
    const { data, error } = await need().from("comments").select("*, author:profiles!comments_author_id_fkey(*)").in("post_id", ids).order("created_at");
    if (error) throw error;
    return data;
  },
};
