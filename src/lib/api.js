...
export const posts = {
  ...
  create: async ({ text, imageUrl, images, poll, scheduled_for }) => {
    const sb = need();
    const uid = (await sb.auth.getUser()).data.user.id;
    const arr = (images && images.length) ? images : (imageUrl ? [imageUrl] : null);
    const { data, error } = await sb
      .from("posts")
      .insert({ author_id: uid, text, image_url: arr ? arr[0] : null, images: arr, has_poll: !!poll, scheduled_for: scheduled_for || null })
      .select()
      .single();
    if (error) throw error;
    if (poll && data) {
