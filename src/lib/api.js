...
export const groups = {
  list: async () => {
    const { data, error } = await need()
      .from("groups")
      .select("*, group_members(user_id), group_posts(*, author:profiles(*))")
      .order("created_at", { ascending: false });
    if (error) throw error;
    return data;
  },
  ...
  post: async (groupId, { text, imageUrl }) => {
    const sb = need();
    const uid = (await sb.auth.getUser()).data.user.id;
    const { data, error } = await sb.from("group_posts").insert({ group_id: groupId, author_id: uid, text, image_url: imageUrl }).select("*, author:profiles(*)").single();
    if (error) throw error;
    return data;
  },
};
...
export const events = {
  list: async () => {
    const { data, error } = await need()
      .from("events")
      .select("*, host:profiles(*), event_rsvps(user_id, status)")
...
export const forum = {
  list: async () => {
    const { data, error } = await need()
      .from("threads")
      .select("*, author:profiles(*), thread_replies(*, author:profiles(*)), thread_votes(user_id)")
      .order("created_at", { ascending: false });
    if (error) throw error;
    return data;
  },
  ...
  reply: async (threadId, text) => {
    const sb = need();
    const uid = (await sb.auth.getUser()).data.user.id;
    const { data, error } = await sb.from("thread_replies").insert({ thread_id: threadId, author_id: uid, text }).select("*, author:profiles(*)").single();
    if (error) throw error;
    return data;
  },
