...
import { Drawer, NavStrip } from "./ui/social";
...
export default function App() {
  ...
  const { following, followingRef, blockedIds, mutedIds, closeFriends, collections, followerCounts, isFollowing, toggleFollow, onBlock, onUnblock, onMute, onUnmute, onToggleCloseFriend, onExportData, onSetBirthday, onToggleShowProfileVisits, onToggleIsPrivate, onDeleteAccount, onCreateCollection, onAssignCollection } = social;
  ...
  const openProfile = (id) => {
    setDrawerOpen(false); setProfileId(id); setTab("profile"); feed.loadUserPosts(id);
    setFollowsMe(false);
    if (id !== ME && hasSupabase) followsApi.isFollowedBy(id).then(setFollowsMe).catch(() => {});
  };
    ...
    <div className="min-h-screen w-full" style={{ color: C.ink, fontFamily: BODY, backgroundColor: C.paper, backgroundImage: `radial-gradient(${C.grid} 1px, transparent 1px)`, backgroundSize: "22px 22px" }}>
          ...
          <div className={fullBleed ? "" : "slidein"} key={tab + (profileId || "") + (feed.activeTag || "")}>
            {tab === "home" && (
              <>
                ...
                <NavStrip nav={NAV} onNav={goTab} onCreate={() => setCreateOpen(true)} flash={flash} tab={tab} onSettings={() => setSettingsOpen(true)} onSignOut={() => authApi.signOut().catch(dbErr("გასვლა"))} />
      ...
      <nav className="mz-nav md:hidden fixed bottom-0 inset-x-0 z-30 flex items-center justify-around px-2 pt-1.5" style={{ background: C.surface + "f0", backdropFilter: "blur(16px)", borderTop: `1px solid ${C.line}`, paddingBottom: "max(0.5rem, env(safe-area-inset-bottom))" }}>
        {BOTTOM.map(k => {
          ...
          if (k === "create") return <button key={k} onClick={() => setCreateOpen(true)} aria-label={t("action.create")} className="rounded-2xl flex items-center justify-center -mt-1 active:scale-90 transition" style={{ width: 46, height: 46, backgroundImage: GBRAND, boxShadow: SH.glow }}><Plus size={26} color="#fff" /></button>;
          const active = tab === k;
          return <button key={k} onClick={() => goTab(k)} aria-label={n.label} className="relative flex flex-col items-center gap-1 px-3 py-1 active:scale-90" style={{ color: active ? C.accent : C.faint }}>{k === "profile" ? <Avatar id={ME} size={26} /> : <n.icon size={25} fill={active && k === "home" ? C.accent : "none"} />}{active && k !== "profile" && <span className="absolute -bottom-0.5 rounded-full" style={{ width: 4, height: 4, backgroundImage: GBRAND }} />}{n.badge > 0 && <span className="absolute top-0 right-1.5 rounded-full text-white flex items-center justify-center" style={{ minWidth: 16, height: 16, padding: "0 4px", background: C.like, fontFamily: MONO, fontSize: 10, fontWeight: 700 }}>{n.badge > 9 ? "9+" : n.badge}</span>}</button>;
        })}
      </nav>
