...
function Messages({ convos, openId, setOpenId, onSend, onReply, onCreateConvo, onOpenProfile, live, onMenu }) {
  ...
  if (cv) {
    ...
    return (
      <div className="fixed inset-0 z-40 flex justify-center" style={{ background: C.paper }}>
        <div className="flex flex-col w-full max-w-[600px]" style={{ height: "100%", background: C.paper, borderLeft: `1px solid ${C.line}`, borderRight: `1px solid ${C.line}` }}>
          ...
          <div className="flex items-center gap-2 px-2.5 py-2.5 shrink-0" style={{ background: C.surface, borderTop: `1px solid ${C.line}`, paddingBottom: (vph != null && (window.innerHeight - vph) > 120) ? "0.625rem" : "max(0.625rem, env(safe-area-inset-bottom))" }}>
            {recording ? (
              <>
  ...
  return (
    <div className="pb-28 md:pb-8">
      <div className="flex items-center gap-2 px-3 py-3 shrink-0" style={{ background: C.surface, borderBottom: `1px solid ${C.line}`, paddingTop: "max(0.75rem, env(safe-area-inset-top))" }}>
        <button onClick={onMenu} className="md:hidden rounded-full flex items-center justify-center active:scale-90" style={{ width: 40, height: 40, color: C.ink2 }}><Menu size={23} /></button>
        <span className="font-bold text-[19px]" style={{ color: C.ink, fontFamily: DISPLAY }}>შეტყობინებები</span>
        <div className="flex-1" />
        <button onClick={() => setPicker("new")} className="rounded-full flex items-center justify-center active:scale-90" style={{ width: 40, height: 40, backgroundImage: GBRAND, color: "#fff", boxShadow: SH.glow }}><Plus size={21} /></button>
      </div>
      ...
      <div className="mx-auto flex w-full max-w-[1100px]">
        ...
        <main className="flex-1 min-w-0 max-w-[600px] mx-auto" style={{ borderRight: `1px solid ${C.line}` }}>
          ...
          <div className={fullBleed ? "" : "fadein"} key={tab + (profileId || "") + (activeTag || "")}>
            ...
            {tab === "messages" && <Messages convos={convos} openId={openConvoId} setOpenId={setOpenConvoId} onSend={onSendMsg} onReply={onReply} onCreateConvo={onCreateConvo} onOpenProfile={openProfile} live={live} onMenu={() => setDrawerOpen(true)} />}
