...
/* ─────────────────────────  SEED DATA  ───────────────────────── */
const FALLBACK_USER = { id: "", name: "მომხმარებელი", handle: "user", bio: "", followers: 0, following: 0, online: false, verified: false, admin: false, avatar: null };
const _users = {};
...
function Profile({ userId, posts, xp, meProfile, following, followerCounts, onToggleFollow, onMessage, onOpenList, onSettings, flash, onBack, onTag, onLike, onReact, onSave, onComment, onPollVote, onReport, onRemove, onOpenProfile, isAdmin, onUploadAvatar }) {
...
/* ─────────────────────────  DRAWER  ───────────────────────── */
function Drawer({ open, onClose, nav, onNav, onCreate, flash, tab, mode, setMode, xp, onSettings, onSignOut }) {
...
function mergeProfile(p) {
  ...
  USERS[p.id] = {
    ...
    online: true, verified: !!p.verified, admin: !!p.is_admin, avatar: p.avatar_url || null,
  };
}
...
function HighlightCreate({ onClose, onCreated, flash }) {
  ...
  return (
    <div className="fixed inset-0 z-[210] flex items-end justify-center" onClick={onClose} style={{ background: "rgba(0,0,0,.55)" }}>
      <div onClick={e => e.stopPropagation()} className="w-full max-w-[440px] rounded-t-3xl p-5" style={{ background: C.paper, paddingBottom: "max(20px, env(safe-area-inset-bottom))" }}>
        <div className="flex justify-center pb-3"><div style={{ width: 38, height: 4, borderRadius: 2, background: C.line }} /></div>
        ...
        <button onClick={save} disabled={!title.trim() || busy} className="w-full py-3 rounded-xl font-bold text-[15px] active:scale-95" style={{ backgroundImage: GBRAND, color: "#fff", opacity: (!title.trim() || busy) ? 0.5 : 1 }}>{busy ? "..." : "შექმნა"}</button>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const [meProfile, setMeProfile] = useState({ name: "", bio: "", avatar: null });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setMeProfile({ name: USERS[ME].name, bio: USERS[ME].bio, avatar: USERS[ME].avatar });
        await loadFeed();
  ...
  const loadReels = async () => { if (!hasSupabase) return; try { let rows; try { rows = await reelsApi.list(); } catch (em) { rows = await hydrateAuthors(await reelsApi.listPlain(), "author_id", "author"); } let liked = new Set(); try { liked = new Set((await reelsApi.mine()).map(x => x.reel_id)); } catch (e) {} setReels(rows.map(r => mapDbReel(r, liked))); } catch (e) { console.error("reels:", e); setDbError("reels: " + (e.message || JSON.stringify(e)) + (e.hint ? " · hint: " + e.hint : "") + (e.code ? " · code: " + e.code : "")); } };
  ...
  const onComment = (id, text) => { setPosts(ps => ps.map(p => p.id === id ? { ...p, comments: [...p.comments, { id: "c" + Date.now(), authorId: ME, text, time: "ახლა" }] } : p)); gainXp(5); commentsApi.add(id, text).catch(dbErr("კომენტარი")); };
  ...
  const onNewListing = (d) => { flash("განცხადება დაიდო 🛍️"); marketApi.create({ title: d.title, price: d.price, description: d.desc, category: d.cat, image_url: d.image, location: "თბილისი" }).then(loadListings).catch(dbErr("განცხადება")); };
...
/* ─────────────────────────  REELS  ───────────────────────── */
function ReelCard({ r, onLike, onSave, onOpenProfile, flash, onComments }) {
  ...
  return (
    <div className="relative w-full" style={{ height: "100dvh", scrollSnapAlign: "start" }}>
      {r.video ? <video ref={videoRef} src={r.video} className="absolute inset-0 w-full h-full" style={{ objectFit: "cover" }} loop muted playsInline autoPlay preload="auto" /> : <Pic src={r.image} grad={GRADS[hashIdx(r.id, GRADS.length)]} className="absolute inset-0 w-full h-full" />}
  ...
  return (
    <div className="relative" style={{ height: "100dvh", background: "#000" }}>
      <div className="no-scrollbar overflow-y-scroll h-full" style={{ scrollSnapType: "y mandatory" }}>
        {reels.map(r => <ReelCard key={r.id} r={r} onLike={onLike} onSave={onSave} onOpenProfile={onOpenProfile} flash={flash} onComments={onComments} />)}
      </div>
