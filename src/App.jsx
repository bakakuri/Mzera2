      ...
      <div className="mx-auto flex w-full max-w-[1100px]">
        ...
        <main className="flex-1 min-w-0 max-w-[600px] mx-auto" style={{ borderRight: `1px solid ${C.line}` }}>
          ...
          <div className={fullBleed ? "" : "slidein"} key={tab + (profileId || "") + (feed.activeTag || "")}>
            ...
            {tab === "market" && <Market listings={market.listings} onSave={market.onListingSave} onNew={market.onNewListing} onMessage={chat.onMessageUser} onOpenProfile={openProfile} flash={flash} live={live} onOrder={market.onOrder} getReviews={market.getReviews} onAddReview={market.addReviewApi} onUpload={(f) => uploadImage(f, "market")} onEdit={market.onEditListing} onDelete={market.onDeleteListing} sentinelRef={market.listSentinelRef} hasMore={market.listMore} loadingMore={market.listLoadingMore} pendingOpen={market.pendingListing} clearPending={() => market.setPendingListing(null)} />}
