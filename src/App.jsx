...
function MapView({ onMessage, onMenu, onOpenProfile }) {
  ...
  const [pins, setPins] = useState([]); const [sharing, setSharing] = useState(false); const [loading, setLoading] = useState(true); const [busy, setBusy] = useState(false); const [geoErr, setGeoErr] = useState("");
  const load = async () => {
    ...
    try { const m = await locationsApi.mine(); setSharing(!!(m && m.shared)); } catch (e) {}
  };
  ...
  const toggleShare = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => { locationsApi.share(pos.coords.latitude, pos.coords.longitude).then(() => setSharing(true)).then(load).catch(() => setGeoErr("შენახვა ვერ მოხერხდა")).finally(() => setBusy(false)); },
  ...
  const positioned = (() => {
    ...
    const dLat = (maxLat - minLat) || 0.02, dLng = (maxLng - minLng) || 0.02;
    minLat -= dLat * 0.2; maxLat += dLat * 0.2; minLng -= dLng * 0.2; maxLng += dLng * 0.2;
    const sLat = (maxLat - minLat) || 1, sLng = (maxLng - minLng) || 1;
    return pins.map(p => ({ ...p, x: 12 + (p.lng - minLng) / sLng * 76, y: 12 + (maxLat - p.lat) / sLat * 70 }));
  })();
  const sf = sel ? positioned.find(f => f.user_id === sel) : null;
  return (
    <div className="relative overflow-hidden" style={{ height: "100dvh", background: C.mapBase }}>
      <div className="absolute inset-0" style={{ transform: `scale(${zoom})`, transformOrigin: "center", transition: "transform .35s cubic-bezier(.4,0,.2,1)" }}>
        ...
        {positioned.map(f => { const isMe = f.user_id === ME; const [a, b] = GRADS[hashIdx(f.user_id, GRADS.length)]; return (
          <button key={f.user_id} onClick={() => setSel(f.user_id)} className="absolute active:scale-95 transition" style={{ left: `${f.x}%`, top: `${f.y}%`, transform: "translate(-50%,-100%)" }}>
            <div className="relative" style={{ filter: "drop-shadow(0 4px 8px rgba(0,0,0,.35))" }}>
              <div className="rounded-full p-[3px]" style={{ background: (sel === f.user_id || isMe) ? GBRAND : `linear-gradient(140deg,${a},${b})` }}><div className="rounded-full p-[2px]" style={{ background: C.surface }}><Avatar id={f.user_id} size={sel === f.user_id ? 48 : 40} /></div></div>
              <div className="absolute left-1/2 -bottom-1.5 w-3 h-3" style={{ background: C.surface, transform: "translateX(-50%) rotate(45deg)" }} />
              {isMe && <span className="absolute -top-1 left-1/2 -translate-x-1/2 px-1.5 rounded-full text-white" style={{ background: C.accent, fontSize: 9, fontWeight: 700 }}>შენ</span>}
            </div>
          </button>
        ); })}
        {!loading && positioned.length === 0 && (
          <div className="absolute inset-0 flex flex-col items-center justify-center px-10 text-center" style={{ pointerEvents: "none" }}>
            <div className="rounded-3xl flex items-center justify-center mb-4" style={{ width: 72, height: 72, background: C.surface, boxShadow: SH.card }}><Map size={32} style={{ color: C.accent }} /></div>
            <div className="text-[15px] font-bold" style={{ color: C.ink }}>რუკა ცარიელია</div>
            <div className="text-[13px] mt-1" style={{ color: C.muted }}>გააზიარე შენი ლოკაცია ან დაელოდე — აქ მხოლოდ ის ხალხი ჩანს ვისაც მიჰყვები და ვინც ლოკაცია გააზიარა.</div>
          </div>
        )}
      </div>
      ...
      <div className="absolute top-0 inset-x-0 p-3 flex items-center gap-2" style={{ background: `linear-gradient(180deg, ${C.paper}cc, transparent)` }}>
        ...
        <div className="flex-1 flex items-center gap-2 px-4 py-2.5 rounded-full" style={{ background: C.surface, boxShadow: SH.card }}>
          ...
          <div className="flex-1" /><span className="flex items-center gap-1" style={{ color: C.online }}><Users size={14} /><Mono className="text-[13px] font-bold">{positioned.length} გააზიარა</Mono></span>
        </div>
      </div>

      <div className="absolute left-3 right-3 flex flex-col items-center" style={{ bottom: sf ? 200 : 96 }}>
      ...
      <div className="absolute right-3 flex flex-col gap-2" style={{ bottom: sf ? 270 : 160 }}>
...
function Messages({ convos, openId, setOpenId, onSend, onReply, onCreateConvo, onOpenProfile, live }) {
  ...
  if (cv) {
    ...
    return (
      <div className="fixed inset-0 z-40 flex justify-center" style={{ background: C.paper, height: vph ? vph + "px" : "100dvh" }}>
        <div className="flex flex-col w-full max-w-[600px]" style={{ height: vph ? vph + "px" : "100dvh", background: C.paper, borderLeft: `1px solid ${C.line}`, borderRight: `1px solid ${C.line}` }}>
...
function Profile({ userId, posts, savedPosts, reels, xp, meProfile, following, followerCounts, onToggleFollow, onMessage, onOpenList, onSettings, flash, onBack, onTag, onLike, onReact, onSave, onComment, onPollVote, onReport, onRemove, onOpenProfile, isAdmin, onUploadAvatar, onUploadCover, onOpenReels }) {
  const u = USERS[userId]; const isMe = userId === ME; const [tab, setTab] = useState("grid"); const [sel, setSel] = useState(null);
  ...
  return (
    <div className="pb-28 md:pb-10">
      ...
      {tab === "saved" && ((saved.length || savedReels.length) ? <div className="pt-2">
        {savedReels.length > 0 && <div className="px-1"><div className="flex items-center gap-1.5 px-3 py-2 text-[13px] font-bold" style={{ color: C.muted }}><Film size={15} style={{ color: C.accent }} /> შენახული Reels</div><div className="grid grid-cols-3 gap-1">{savedReels.map(r => <button key={r.id} onClick={() => onOpenReels && onOpenReels()} className="relative active:opacity-80"><Pic src={r.image} grad={GRADS[hashIdx(r.id, GRADS.length)]} round={10} style={{ aspectRatio: "9/16" }} /><span className="absolute top-1.5 right-1.5" style={{ color: "#fff", filter: "drop-shadow(0 1px 2px rgba(0,0,0,.6))" }}><Play size={16} fill="#fff" /></span></button>)}</div></div>}
        {saved.length > 0 && <div className="space-y-4 px-3 pt-3">{savedReels.length > 0 && <div className="flex items-center gap-1.5 text-[13px] font-bold pt-1" style={{ color: C.muted }}><Bookmark size={15} style={{ color: C.accent }} /> შენახული პოსტები</div>}{saved.map(p => <PostCard key={p.id} post={p} onLike={onLike} onReact={onReact} onSave={onSave} onComment={onComment} onPollVote={onPollVote} onTag={onTag} onReport={onReport} onRemove={onRemove} onOpenProfile={onOpenProfile} isAdmin={isAdmin} />)}</div>}
      ...
      {selPost && (
        <div className="fixed inset-0 z-[60] flex items-start justify-center overflow-y-auto" style={{ background: "rgba(0,0,0,.55)" }} onClick={() => setSel(null)}>
          <div className="w-full max-w-[540px] min-h-full md:min-h-0 md:my-6" style={{ background: C.paper }} onClick={e => e.stopPropagation()}>
            ...
            <div className="p-3"><PostCard post={selPost} onLike={onLike} onReact={onReact} onSave={onSave} onComment={onComment} onPollVote={onPollVote} onTag={onTag} onReport={onReport} onRemove={onRemove} onOpenProfile={onOpenProfile} isAdmin={isAdmin} /></div>
          </div>
        </div>
      )}
    </div>
...
/* ─────────────────────────  ADMIN  ───────────────────────── */
function Admin({ reports, posts, userCount, postCount, online, onResolve, onRemovePost }) {
...
function mergeProfile(p) {
  ...
  USERS[p.id] = {
    ...
    xp: p.xp ?? 0, location: p.location || "", website: p.website || "",
    online: true, verified: !!p.verified, admin: !!p.is_admin, avatar: p.avatar_url || null, cover: p.cover_url || null,
  };
}
      ...
      <div className="mx-auto flex w-full max-w-[1100px]">
        ...
        <main className="flex-1 min-w-0 max-w-[600px] mx-auto" style={{ borderRight: `1px solid ${C.line}` }}>
          ...
          <div className={fullBleed ? "" : "fadein"} key={tab + (profileId || "") + (activeTag || "")}>
            ...
            {tab === "groups" && <Groups groups={groups} events={events} onJoin={onJoinGroup} onRsvp={onRsvp} onOpenProfile={openProfile} onMessage={onMessageUser} live={live} onGroupPost={onGroupPost} onUpload={(f) => uploadImage(f, "groups")} onCreateGroup={onCreateGroup} onCreateEvent={onCreateEvent} />}
            ...
            {tab === "admin" && <Admin reports={reports} posts={posts} userCount={userCount} postCount={postCount} online={onlineCount} onResolve={onResolve} onRemovePost={onRemovePost} />}
          </div>
        </main>
...
function Groups({ groups, events, onJoin, onRsvp, onOpenProfile, onMessage, live, onGroupPost, onUpload, onCreateGroup, onCreateEvent }) {
