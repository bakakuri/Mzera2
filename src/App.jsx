...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      loadNotifs().catch(() => {}); loadConvos().catch(() => {}); loadStories().catch(() => {}); loadReels().catch(() => {}); loadListings().catch(() => {}); loadFilms().catch(() => {}); loadFilmWatch().catch(() => {}); loadMusic().catch(() => {}); loadGroups().catch(() => {}); loadEvents().catch(() => {}); loadThreads().catch(() => {}); loadShareCounts().catch(() => {});
    })();
    return () => { cancelled = true; };
  }, [session]);
  ...
  const openPromo = (promo) => {
    if (promo.kind === "group") { setPendingGroup(promo.id); setTab("groups"); }
    ...
    else if (promo.kind === "game") { if (promo.id === "bura") setBuraOpen(true); else setNardiOpen(true); }
  };
  const feedItems = (() => {
    ...
    const promos = [
      (() => { const p = pickTop(groups.filter(g => !g.joined), g => g.members); return p && { kind: "group", id: p.id, image: p.cover, title: p.name, subtitle: `${p.members} წევრი · გაწევრიანდი`, cta: "გაწევრიანება" }; })(),
      (() => { const p = pickTop(films, f => new Date(f.createdAt || 0).getTime()); return p && { kind: "film", id: p.id, image: p.poster, title: p.title, subtitle: [p.year, p.genre].filter(Boolean).join(" · "), cta: "ნახვა" }; })(),
      (() => { const p = pickTop(songs, s => s.plays); return p && { kind: "song", id: p.id, image: p.cover, title: p.title, subtitle: `${p.artist || "უცნობი შემსრულებელი"} · ${p.plays} მოსმენა`, cta: "მოსმენა" }; })(),
      ...
      <div className="mx-auto flex w-full max-w-[1100px]">
        ...
        <main className="flex-1 min-w-0 max-w-[600px] mx-auto" style={{ borderRight: `1px solid ${C.line}` }}>
          ...
          <div className={fullBleed ? "" : "slidein"} key={tab + (profileId || "") + (activeTag || "")}>
            ...
            {tab === "admin" && <Admin reports={reports} posts={posts} allUsers={allUsers} userCount={userCount} postCount={postCount} online={onlineCount} stats={adminStats} onResolve={onResolve} onRemovePost={onRemovePost} onSetVerified={onSetVerified} onSetAdmin={onSetAdmin} onOpenProfile={openProfile} onBanUser={onBanUser} onGrantXp={onGrantXp} onSetXp={onSetXp} onDeleteUser={onDeleteUser} onBroadcast={onBroadcast} pendingPublic={pendingPublic} onReviewPublic={onReviewPublic} listings={listings} threads={threads} reels={reels} onDeleteListing={onDeleteListing} onDeleteThread={onDeleteThread} onDeleteReel={onReelDelete} onEditListing={onEditListing} onEditThread={onEditThread} groups={groups} events={events} films={films} songs={songs} onEditGroup={onEditGroup} onDeleteGroup={onDeleteGroup} onEditEvent={onEditEvent} onDeleteEvent={onDeleteEvent} onEditFilm={onEditFilm} onDeleteFilm={onDeleteFilm} onEditSong={onEditSong} onDeleteSong={onDeleteSong} />}
          </div>
          </Suspense>
        </main>
