...
function StoryViewer({ story, onClose, onDone, flash }) {
  ...
  const [reply, setReply] = useState(""); const [floats, setFloats] = useState([]); const [liked, setLiked] = useState(false);
  useEffect(() => { setProg(0); setLiked(false); const t = setInterval(() => { if (pausedRef.current) return; setProg(p => { if (p >= 100) { if (idx < items.length - 1) { setIdx(i => i + 1); return 0; } clearInterval(t); onDone(story.id); onClose(); return 100; } return p + 2; }); }, 60); return () => clearInterval(t); }, [idx]);
  const pushFloat = (text, heart) => { const id = Date.now() + Math.random(); setFloats(f => [...f, { id, text, heart }]); setTimeout(() => setFloats(f => f.filter(x => x.id !== id)), 4200); };
  const send = () => { const t = reply.trim(); if (!t) return; pushFloat(t, false); setReply(""); setPaused(false); };
  const toggleLike = () => { setLiked(l => { if (!l) pushFloat("", true); return !l; }); };
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center" style={{ background: "rgba(6,7,12,.96)" }}>
      <div className="relative w-full max-w-[440px] h-full sm:h-[92vh] sm:rounded-3xl overflow-hidden">
        ...
        <div className="absolute inset-x-0 pointer-events-none flex flex-col-reverse items-start gap-1.5 px-3" style={{ bottom: 72, maxHeight: 240, overflow: "hidden" }}>
          {floats.map(f => <div key={f.id} style={{ animation: "floatUp 4.1s ease-out forwards" }}>{f.heart ? <Heart size={30} fill="#ff3b5c" color="#ff3b5c" style={{ filter: "drop-shadow(0 2px 6px rgba(0,0,0,.5))" }} /> : <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full" style={{ background: "rgba(0,0,0,.55)", backdropFilter: "blur(4px)" }}><Avatar id={ME} size={20} /><span className="text-white text-[13px]">{f.text}</span></span>}</div>)}
        </div>
...
function Messages({ convos, openId, setOpenId, onSend, onReply, onCreateConvo, onOpenProfile, live, onMenu, groups, onOpenGroup }) {
  ...
  if (cv) {
    ...
    return (
      <div className="fixed left-0 right-0 z-40" style={{ background: C.paper, top: 56, bottom: "calc(56px + env(safe-area-inset-bottom))" }}>
        <div className="absolute inset-0 flex flex-col" style={{ maxWidth: 600, margin: "0 auto", background: C.paper, borderLeft: `1px solid ${C.line}`, borderRight: `1px solid ${C.line}` }}>
      ...
      <div className="mx-auto flex w-full max-w-[1100px]">
        ...
        <main className="flex-1 min-w-0 max-w-[600px] mx-auto" style={{ borderRight: `1px solid ${C.line}` }}>
          ...
          <div className={fullBleed ? "" : "fadein"} key={tab + (profileId || "") + (activeTag || "")}>
            ...
            {tab === "messages" && <Messages convos={convos} openId={openConvoId} setOpenId={setOpenConvoId} onSend={onSendMsg} onReply={onReply} onCreateConvo={onCreateConvo} onOpenProfile={openProfile} live={live} onMenu={() => setDrawerOpen(true)} groups={groups} onOpenGroup={(id) => { setPendingGroup(id); setTab("groups"); }} />}
