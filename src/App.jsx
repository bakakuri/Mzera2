...
export default function App() {
  ...
  const { following, followingRef, blockedIds, mutedIds, closeFriends, collections, followerCounts, isFollowing, toggleFollow, onBlock, onUnblock, onMute, onUnmute, onToggleCloseFriend, onExportData, onSetBirthday, onDeleteAccount, onCreateCollection, onAssignCollection } = social;
  ...
  const notifications = useNotifications({ session, live });
  ...
  const goBackOneLevel = () => {
    ...
    if (feed.postView) { feed.setPostView(null); return true; }
  ...
  return (
    <div className="min-h-screen w-full" style={{ color: C.ink, fontFamily: BODY, backgroundColor: C.paper, backgroundImage: `radial-gradient(${C.grid} 1px, transparent 1px)`, backgroundSize: "22px 22px" }}>
      ...
      <div className="mx-auto flex w-full max-w-[1100px]">
        ...
        <main className="flex-1 min-w-0 max-w-[600px] mx-auto" style={{ borderRight: `1px solid ${C.line}` }}>
          ...
          <div className={fullBleed ? "" : "slidein"} key={tab + (profileId || "") + (feed.activeTag || "")}>
            ...
            {tab === "market" && <Market listings={market.listings} onSave={market.onListingSave} onNew={market.onNewListing} onMessage={chat.onMessageUser} onOpenProfile={openProfile} flash={flash} live={live} onOrder={market.onOrder} getReviews={market.getReviews} onAddReview={market.addReviewApi} onUpload={(f) => uploadImage(f, "market")} onEdit={market.onEditListing} onDelete={market.onDeleteListing} sentinelRef={market.listSentinelRef} hasMore={market.listMore} loadingMore={market.listLoadingMore} />}
            {tab === "games" && <GamesList onOpenBura={() => games.setBuraOpen(true)} onOpenNardi={() => games.setNardiOpen(true)} />}
            {tab === "movies" && <Movies films={movies.films} watch={movies.filmWatch} onNew={movies.onNewFilm} onEdit={movies.onEditFilm} onDelete={movies.onDeleteFilm} onOpenProfile={openProfile} flash={flash} onUpload={(f) => uploadImage(f, "films")} onUploadVideo={(f) => storageApi.upload(f, "films")} getReviews={movies.getFilmReviews} onAddReview={movies.addFilmReviewApi} onSetWatch={movies.onSetFilmWatch} onClearWatch={movies.onClearFilmWatch} sentinelRef={movies.filmSentinelRef} hasMore={movies.filmMore} loadingMore={movies.filmLoadingMore} />}
            {tab === "music" && <MusicPage songs={music.songs} nowPlaying={music.nowPlaying} isPlaying={music.isPlaying} onPlay={music.playSong} onNew={music.onNewSong} onEdit={music.onEditSong} onDelete={music.onDeleteSong} onUpload={(f) => uploadImage(f, "music")} onUploadAudio={(f) => storageApi.upload(f, "music")} />}
            {tab === "map" && <MapView onMessage={chat.onMessageUser} onMenu={() => setDrawerOpen(true)} onOpenProfile={openProfile} />}
            {tab === "reels" && <Reels reels={reelsHook.reels} onLike={reelsHook.onReelLike} onSave={reelsHook.onReelSave} onView={reelsHook.onReelView} onOpenProfile={openProfile} onMenu={() => setDrawerOpen(true)} flash={flash} onCreate={() => reelsHook.setReelCreateOpen(true)} onComments={reelsHook.openReelComments} sentinelRef={reelsHook.reelsSentinelRef} hasMore={reelsHook.reelsMore} loadingMore={reelsHook.reelsLoadingMore} />}
            {tab === "groups" && <Groups groups={groups.groups} events={groups.events} onJoin={groups.onJoinGroup} onRsvp={groups.onRsvp} onOpenProfile={openProfile} onMessage={chat.onMessageUser} live={live} onGroupPost={groups.onGroupPost} onUpload={(f) => uploadImage(f, "groups")} onUploadVideo={(f) => storageApi.upload(f, "groups")} onCreateGroup={groups.onCreateGroup} onCreateEvent={groups.onCreateEvent} pendingOpen={groups.pendingGroup} clearPending={() => groups.setPendingGroup(null)} onEditPost={groups.onEditGroupPost} onDeletePost={groups.onDeleteGroupPost} onEditGroup={groups.onEditGroup} onDeleteGroup={groups.onDeleteGroup} onEditEvent={groups.onEditEvent} onDeleteEvent={groups.onDeleteEvent} allPosts={feed.posts} loadGroupPosts={groups.mergeGroupPosts} loadMembers={(gid) => groupsApi.members(gid).then(rows => { rows.forEach(r => { if (r.profile) mergeProfile(r.profile); }); return rows; })} onApproveMember={groups.onApproveMember} onKickMember={groups.onKickMember} onSetGroupPrivate={groups.onSetGroupPrivate} taggable={following} postProps={{ onLike: feed.onLike, onReact: feed.onReact, onSave: feed.onSave, onComment: feed.onComment, onPollVote: feed.onPollVote, onTag, onReport: admin.onReport, onRemove: feed.onRemovePost, onOpenProfile: openProfile, isAdmin: me.admin, onEdit: feed.onEditPost, onDelete: feed.onDeletePost, onEditComment: feed.onEditComment, onDeleteComment: feed.onDeleteComment, onLikeComment: feed.onLikeComment, onRepost: feed.onRepost, onReactors: (pid) => reactionsApi.listForPost(pid) }} />}
            ...
            {tab === "notifications" && <Notifications notifs={notifications.notifs} onOpenProfile={openProfile} onOpenPost={feed.openPost} isFollowing={isFollowing} onToggleFollow={toggleFollow} />}
            ...
            {tab === "profile" && <Profile userId={profileId || ME} posts={feed.posts} savedPosts={feed.savedPosts} reels={reelsHook.reels} xp={xp} meProfile={meProfile} following={following} followerCounts={followerCounts} onToggleFollow={toggleFollow} onMessage={chat.onMessageUser} onOpenList={(type, uid) => setListView({ type, userId: uid })} onSettings={() => setSettingsOpen(true)} flash={flash} onBack={() => goTab("home")} onTag={onTag} onLike={feed.onLike} onReact={feed.onReact} onSave={feed.onSave} onComment={feed.onComment} onPollVote={feed.onPollVote} onReport={admin.onReport} onRemove={feed.onRemovePost} onOpenProfile={openProfile} isAdmin={me.admin} onUploadAvatar={onChangeAvatar} onUploadCover={onChangeCover} onOpenReels={() => goTab("reels")} onAddReel={() => reelsHook.setReelCreateOpen(true)} onReelDelete={reelsHook.onReelDelete} onReelEdit={reelsHook.onReelEdit} onEditPost={feed.onEditPost} onDeletePost={feed.onDeletePost} onEditComment={feed.onEditComment} onDeleteComment={feed.onDeleteComment} blocked={blockedIds.includes(profileId || ME)} muted={mutedIds.includes(profileId || ME)} onBlock={onBlock} onUnblock={onUnblock} onMute={onMute} onUnmute={onUnmute} closeFriend={closeFriends.includes(profileId || ME)} onToggleCloseFriend={onToggleCloseFriend} collections={collections} onCreateCollection={onCreateCollection} onAssignCollection={onAssignCollection} />}
            ...
            {tab === "admin" && <Admin reports={admin.reports} posts={feed.posts} allUsers={admin.allUsers} userCount={admin.userCount} postCount={admin.postCount} online={onlineCount} stats={admin.adminStats} onResolve={admin.onResolve} onRemovePost={feed.onRemovePost} onSetVerified={admin.onSetVerified} onSetAdmin={admin.onSetAdmin} onOpenProfile={openProfile} onBanUser={admin.onBanUser} onGrantXp={admin.onGrantXp} onSetXp={admin.onSetXp} onDeleteUser={admin.onDeleteUser} onBroadcast={admin.onBroadcast} pendingPublic={admin.pendingPublic} onReviewPublic={admin.onReviewPublic} listings={market.listings} threads={forum.threads} reels={reelsHook.reels} onDeleteListing={market.onDeleteListing} onDeleteThread={forum.onDeleteThread} onDeleteReel={reelsHook.onReelDelete} onEditListing={market.onEditListing} onEditThread={forum.onEditThread} groups={groups.groups} events={groups.events} films={movies.films} songs={music.songs} onEditGroup={groups.onEditGroup} onDeleteGroup={groups.onDeleteGroup} onEditEvent={groups.onEditEvent} onDeleteEvent={groups.onDeleteEvent} onEditFilm={movies.onEditFilm} onDeleteFilm={movies.onDeleteFilm} onEditSong={music.onEditSong} onDeleteSong={music.onDeleteSong} />}
          </div>
          </Suspense>
        </main>
      ...
      {createOpen && <CreateSheet onClose={() => setCreateOpen(false)} onPost={onPost} live={live} taggable={following} myGroups={groups.groups.filter(g => g.joined)} onGroupPost={groups.onGroupPost} onUpload={(f) => uploadImage(f, "posts")} onUploadVideo={(f) => storageApi.upload(f, "posts")} />}
      ...
      {feed.tagView && <div className="slidein fixed inset-0 z-50 overflow-y-auto" style={{ background: C.paper }}>
        <div className="sticky top-0 z-10 flex items-center gap-3 px-4 py-3" style={{ background: C.paper, borderBottom: `1px solid ${C.line}` }}><button onClick={() => feed.setTagView(null)} className="active:scale-90"><ArrowLeft size={22} style={{ color: C.ink }} /></button><div className="font-bold text-[17px]" style={{ color: C.ink, fontFamily: DISPLAY }}>#{feed.tagView}</div></div>
      ...
      {stories.storyEditorOpen && <StoryEditor onClose={() => stories.setStoryEditorOpen(false)} onShare={stories.onAddStory} live={live} onUpload={(f) => uploadImage(f, "stories")} />}
      {reelsHook.reelCreateOpen && <ReelCreate onClose={() => reelsHook.setReelCreateOpen(false)} onPublish={reelsHook.onPublishReel} onUpload={(f) => storageApi.upload(f, "reels")} onUploadThumb={(f) => uploadImage(f, "reels")} flash={flash} />}
      ...
      {searchOpen && <SearchView posts={feed.posts} onOpenProfile={openProfile} onTag={onTag} onClose={() => setSearchOpen(false)} runSearch={feed.runSearch} />}
