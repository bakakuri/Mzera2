...
import { Profile, Notifications, Admin, Drawer, OnlinePage, Progress, SettingsView, Leaderboard, SearchView, SuggestedPeople } from "./ui/social";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        try { const mem = await postsApi.memories(); if (!cancelled && mem.length) setMemories(mem.map(mapDbPost)); } catch (e) {}
        try { const sug = await profilesApi.suggested(); sug.forEach(mergeProfile); if (!cancelled) setSuggested(sug); } catch (e) {}
        await loadNotifs();
        await loadConvos();
        await loadStories();
        await loadReels();
  ...
  return (
    <div className="min-h-screen w-full" style={{ color: C.ink, fontFamily: BODY, backgroundColor: C.paper, backgroundImage: `radial-gradient(${C.grid} 1px, transparent 1px)`, backgroundSize: "22px 22px" }}>
      ...
      {tagView && <div className="fixed inset-0 z-50 overflow-y-auto" style={{ background: C.paper }}>
      ...
      {postView && (() => { const p = posts.find(x => x.id === postView); return <div className="fixed inset-0 z-50 overflow-y-auto" style={{ background: C.paper }}>
